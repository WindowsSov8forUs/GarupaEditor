...
import type { PixiResultScene } from "../backends/pixi/pixiResultScene";
import type { SimulatorResultPreparation, SimulatorResultRecord } from "../host/resultPresentation";
import {
  PixiRendererBackend,
  type PixiInGameControlOverlay,
} from "../backends/pixi/pixiRendererBackend";
import { SimulatorBackendTrace } from "../backends/recordingBackend";
...
export interface AutonomousSimulatorPlatformCapabilities {
  beginResultPreparation(): SimulatorResultPreparation;
...
class ProductionRecipeEngineBuilder implements SimulatorRecipeEngineBuilder {
  ...
  async createFreshEngine(
    ...
    try {
      ...
      const tracing = new SimulatorBackendTrace();
      ...
      const mountedEngine = new MountedSimulatorEngine(
        ...
        async (record, loading) => {
          // One final unit is the destination's first draw beneath Loading.
        ...
        () => this.platform.beginResultPreparation(),
        resourceLease.value,
...
class MountedSimulatorEngine implements SimulatorEngine {
  ...
  constructor(
    ...
    private readonly createResultScene: (record: SimulatorResultRecord, loading: SimulatorResultPreparation) => Promise<SimulatorResult<PixiResultScene>>,
    private readonly beginResultPreparation: () => SimulatorResultPreparation,
  ...
  publishMount(): SimulatorResult<void> {
    ...
    if (mounted.status === "rejected") {
      return integrityFailure(mounted.failure.capability, mounted.failure.boundary);
    }
  ...
  startResultAudio(): SimulatorResult<void> { return this.engine.startResultAudio(); }
  playResultEvaluationSound(failed: boolean): SimulatorResult<void> { return this.engine.playResultEvaluationSound(failed); }
  setResultCountSound(active: boolean): SimulatorResult<void> { return this.engine.setResultCountSound(active); }
  playUiDecisionSound(): SimulatorResult<void> {
    return this.engine.playUiDecisionSound();
  }
  isUiDecisionSoundPlaying(): SimulatorResult<boolean> { return this.engine.isUiDecisionSoundPlaying(); }
  ...
  private resultScene: PixiResultScene | null = null;
  private resultDecision: "exit" | "replay" | null = null;
  private resultCountSoundActive = false;
  private resultPreparation: Promise<SimulatorResult<void>> | null = null;
  private resultLoading: SimulatorResultPreparation | null = null;
  showResult(record: SimulatorResultRecord): Promise<SimulatorResult<void>> {
    this.resultPreparation ??= this.prepareResult(record);
    return this.resultPreparation;
  }
  private async prepareResult(record: SimulatorResultRecord): Promise<SimulatorResult<void>> {
    if (this.resultScene !== null) return ok(undefined);
    let loading: SimulatorResultPreparation | null = null;
    try {
      loading = this.resultLoading = this.beginResultPreparation();
      await loading.ready;
      if (this.disposed) return integrityFailure("simulator.result.preparation-disposed", "Result preparation cannot outlive its session.");
      const scene = await this.createResultScene(record, loading);
      if (scene.status !== "ok") return scene;
      if (this.disposed) {
        scene.value.dispose();
        return integrityFailure("simulator.result.preparation-disposed", "Result preparation cannot outlive its session.");
      }
      const audio = this.engine.startResultAudio();
      if (audio.status !== "ok") { scene.value.dispose(); return audio; }
      this.resultScene = scene.value;
      // Keep the last gameplay frame under Loading until the destination is
      // ready. Result time starts only after this asynchronous handoff.
      for (const child of this.combinedScene.root.children) child.visible = false;
      this.combinedScene.root.addChild(scene.value.root);
      return ok(undefined);
    } catch (error) {
      return integrityFailure("simulator.result.preparation-failed",
        `Result resource or scene preparation failed: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      loading?.finish(this.resultScene !== null && !this.disposed);
      this.resultLoading = null;
    }
  }
  advanceResult(deltaSeconds: number, input: ManualInputFrame | null): SimulatorResult<"stay" | "exit" | "replay"> {
    if (this.resultScene === null) return integrityFailure("simulator.result.scene-not-shown", "A result frame requires an installed result scene.");
    if (this.resultDecision !== null) {
      this.resultScene.advance(deltaSeconds, null);
      const sound = this.engine.isUiDecisionSoundPlaying();
      return sound.status === "ok" ? ok(sound.value ? "stay" : this.resultDecision) : sound;
    }
    const action = this.resultScene.advance(deltaSeconds, input);
    if (action === "success" || action === "failed") {
      const sound = this.engine.playResultEvaluationSound(action === "failed");
      if (sound.status !== "ok") return sound;
    }
    const counting = action !== "decide-exit" && action !== "decide-replay" && this.resultScene.isCounting;
    if (counting !== this.resultCountSoundActive) {
      const sound = this.engine.setResultCountSound(counting);
      if (sound.status !== "ok") return sound;
      this.resultCountSoundActive = counting;
    }
    if (action === "decide-exit" || action === "decide-next" || action === "decide-replay") {
      const sound = this.engine.playUiDecisionSound();
      if (sound.status !== "ok") return sound;
      this.resultDecision = action === "decide-exit" ? "exit" : action === "decide-replay" ? "replay" : null;
    }
    return ok("stay");
  }
  ...
  dispose(): SimulatorResult<void> {
    ...
    const resultDisposed = disposeOwner("result-scene", () => {
      this.resultLoading?.finish(false); this.resultLoading = null;
      this.resultScene?.dispose(); this.resultScene = null; return ok(undefined);
    });
    let result = disposeOwner("engine", () => this.engine.dispose());
    if (result.status === "ok" && resultDisposed.status !== "ok") result = resultDisposed;
...
function validatePlatform(
  ...
  if (
    ...
    typeof platform.beginResultPreparation !== "function" ||
