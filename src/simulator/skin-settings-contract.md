...
The default and non-default routes retain every current serialized system, enabled module, renderer/material/null slot, mesh and texture relation. Runtime random state is allocated per concrete `(owner generation, ParticleSystem component)` from the recovered global manager/manual-seed rules; canonical path/name hashes are forbidden. Shape 0/4/5/8/10 and no-Shape use the recovered `+Z` handoff, Shape TRS and self→immediate→root order. Renderer mode 0/1/4, alignment, pivot/stretch/rotation, UV/custom data, color-space/blend, native sort and aggregate bounds are completed before Pixi; Pixi only executes the immutable primitive.
