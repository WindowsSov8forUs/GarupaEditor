...
The full Reverse source profile contains 133 whole packs / 635 embedded files. The current Standard/MV simulator manifest deliberately removes the three Live2D-only structural-stage packs and pins exactly 130 reachable source packs / 576 embedded PNG/MP3 files. Every launch atomically leases all resolved render/audio packages. For the exact default effect pair, the two unversioned provider TapEffect packages are deliberately not leased: the application capability instead supplies `portable/profiles/default-particle`, whose profile and seven unique PNGs are pinned to the 10.1.4 decoded identities from Reverse `552433d0`. This is an explicit resource route, not a missing/default fallback. Non-default effect selections still consume their complete selected source packages. Every consumed package/file is checked before backend prepare, scene publication, mount or scheduler start; faults never fall back.
...
The default effect route consumes the exact built-in 10.1.4 ordinary/directional profile and PNG identities. Any non-default TapEffect or Directional-effect selection constructs dynamic bundles from its explicitly selected source packages. Both routes retain serialized systems, enabled modules, renderers, materials and textures. A package name or same-looking unversioned PNG cannot satisfy the default identity gate. The portable backend derives each auto-random stream deterministically from canonical resource/system/profile identity; this is a named portable policy, not an original device-random or frame-parity claim.

Reverse `50170414da613b15dd1064addc7cceed0bb991fd` reconciles the visual mapping without replacing the later device-closure simulation oracle: current Skin/particle PNG bytes are sRGB, the locked player active color space is Linear, browser resources remain straight-alpha, normal/additive blending occurs in one linear intermediate target, and the mounted scene performs one final unpremultiply → linear-to-sRGB → repremultiply pass. Encoded-space tint/blend and premultiplied-sRGB upload are forbidden. Particle billboard mapping also preserves authored stretched roll, signed long-axis orientation and the serialized camera-height `m_MaxParticleSize` clamp; the particle layer is nested between Field and Note/HUD ownership instead of drawn as one stage behind the entire ordinary scene. The two authored seven-level particle mip chains remain an explicit portable raster difference because current leased PNG packages contain only the base level; generated mips are not equivalent.
