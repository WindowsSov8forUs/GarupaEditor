...
import { containsSerializedButton, isSerializedTouchRelease, SerializedButtonInput } from "../../components/SerializedButtonInput";
import {
...
const RETRY_CONFIRM_WINDOW_WIDTH = Math.fround(CURRENT_PAUSE_SERIALIZED_GRAPHS.selectable.window.size[0]);
const RETRY_CONFIRM_WINDOW_HEIGHT = Math.fround(CURRENT_PAUSE_SERIALIZED_GRAPHS.selectable.window.size[1]);
const ABORT_CONFIRM_WINDOW_WIDTH = Math.fround(CURRENT_PAUSE_SERIALIZED_GRAPHS.annotated.window.size[0]);
const ABORT_CONFIRM_WINDOW_HEIGHT = Math.fround(CURRENT_PAUSE_SERIALIZED_GRAPHS.annotated.window.size[1]);
const RETRY_CONFIRM_BUTTON_Y = Math.fround(-CURRENT_PAUSE_SERIALIZED_GRAPHS.selectable.buttons[0]!.position[1]);
const ABORT_CONFIRM_BUTTON_Y = Math.fround(-CURRENT_PAUSE_SERIALIZED_GRAPHS.annotated.buttons[0]!.position[1]);
...
export class PauseControlSceneOwner {
  ...
  private readonly buttons = new SerializedButtonInput<PressTarget>();
  ...
  snapshot(
    ...
    return deepFreeze({
      ...
      pressedButtons: Object.freeze(this.buttons.pressedTargets),
      words: VISIBLE_WORDS,
    });
  }
  ...
  private routeModalTouch(
    ...
    if (touch.phase === ManualTouchPhase.Began) {
      this.buttons.press(touch.fingerId, target);
      return ok(undefined);
    }
    if (!isSerializedTouchRelease(touch.phase)) return ok(undefined);
    const pressed = this.buttons.release(touch.fingerId, target);
    if (pressed === null) return ok(undefined);
    commands.push(issue("ui-decide", controlState.mode, layout.surfaceRevision));
  ...
  private targetAt(position: Readonly<{ x: number; y: number }>, layout: PauseControlLayout): PressTarget | null {
    ...
    if (this.state === "pause-menu") {
      if (containsSerializedButton(topLeft, layout.pauseMenu.abortBoundsTopLeft)) return "abort";
      if (containsSerializedButton(topLeft, layout.pauseMenu.retryBoundsTopLeft)) return "retry";
      if (containsSerializedButton(topLeft, layout.pauseMenu.resumeBoundsTopLeft)) return "resume";
      return null;
    }
    if (this.state === "retry-confirm" || this.state === "abort-confirm") {
      ...
      if (containsSerializedButton(topLeft, confirmation.cancelBoundsTopLeft)) return "cancel";
      if (containsSerializedButton(topLeft, confirmation.confirmBoundsTopLeft)) return "confirm";
    }
    return null;
  }
...
export function consumePauseControlCommand(
  ...
  if (command === null || typeof command !== "object" ||
    ...
    return integrityFailure("pause.control.invalid-command", "Only opaque one-use Pause scene commands are accepted.");
  }
...
function phaseName(phase: ManualInputTouch["phase"]): "began" | "moved" | "ended" {
  return phase === ManualTouchPhase.Began ? "began" : isSerializedTouchRelease(phase) ? "ended" : "moved";
}
