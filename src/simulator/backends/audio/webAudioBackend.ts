    ...
    for (let commandIndex = 0; commandIndex < commands.length; commandIndex += 1) {
      ...
      switch (command.kind) {
        ...
        case "bgm.start-result": {
          const previous = this.voices.get("bgm");
          if (previous !== undefined) { this.releaseVoice(previous); this.voices.delete("bgm"); }
          const loop = this.requireLoopResource(command.cue);
          const voice = this.createVoice("bgm", command.cue, "bgm", 0, 0, loop.startSeconds, loop.endSeconds);
          this.beginVoiceFade(voice, 1, command.fade_seconds, false);
          break;
        }
        case "bgm.load":
          this.createVoice(
        ...
        case "se.fade-owned-loop": {
          ...
          if (voice === undefined) throw new Error("missing owned loop voice");
          if (command.duration_bits === "0x00000000") {
            this.releaseVoice(voice); this.voices.delete(voice.voiceKey);
            break;
          }
          this.beginVoiceFade(
            voice,
