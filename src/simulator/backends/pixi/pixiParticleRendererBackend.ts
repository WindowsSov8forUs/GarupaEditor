...
function validateScene(scene: ParticlePixiSceneProfile): ParticleOperationResult<ParticlePixiSceneProfile> {
  ...
  if (scene === null || typeof scene !== "object" || !Number.isSafeInteger(scene.viewportWidth) || scene.viewportWidth <= 0 ||
    ...
    ppu !== Math.fround(scene.viewportHeight / 2) || scene.roundPixels !== false ||
    !Array.isArray(scene.buttonAnchors) || scene.buttonAnchors.length !== 13 ||
    !Array.isArray(scene.buttonOwners) || scene.buttonOwners.length !== 13 || scene.slidePool === undefined ||
    ...
    scene.buttonOwners.some((owner, index) => owner.buttonType !== (index < 7 ? index : index - 7) ||
      owner.isHalfButton !== (index >= 7) ||
    ...
    return particleRejected(
      ...
      "Particle projection requires exact camera, seven full and six half GamePlayButton owners and current eight-slot NoteSlide pool transforms.",
    );
  }
...
function isNativeInstance(value: ParticleInstanceIdentity): boolean {
  ...
  if (value.kind === "game-play-button") {
    return typeof value.isHalfButton === "boolean" && Number.isInteger(value.buttonType) &&
      value.buttonType >= 0 && value.buttonType <= (value.isHalfButton ? 5 : 6) &&
  ...
  return Number.isSafeInteger(value.noteIndex) && value.noteIndex >= 0 &&
    Array.isArray(value.particleSystemSetupScaleFactorsBits) && value.particleSystemSetupScaleFactorsBits.length === 2 &&
    value.particleSystemSetupScaleFactorsBits.every((factor) => positiveBits(factor) !== null) &&
    Number.isFinite(value.absolutePosition) && value.absolutePosition >= 0 && Number.isInteger(value.rangeLength) && value.rangeLength >= 1 &&
