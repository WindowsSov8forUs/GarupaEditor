...
export class PixiGameplayRenderOrder {
  ...
  attachReplacement(node: Container, source: Container): void {
    const order = this.orders.get(source);
    // Unregistered leaves retain their existing logical parent draw order.
    if (order !== undefined) this.attach(node, order);
  }
