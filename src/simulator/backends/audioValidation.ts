...
import resultMedia from "../../data/simulator/resultMediaProfile.json";
...
const FIXED_SE_LOGICAL_IDS = Object.freeze({
  ...
  SE_RESULT_SUCCESS: "sound/common",
  SE_RESULT_FAILED: "sound/common",
  SE_RESULT_SCORE_COUNT: "sound/common",
...
export function validateAndFreezeAudioProfile(
  ...
  if (
    ...
    (profile.resources.length !== FIXED_SE_COUNT + 2 &&
      profile.resources.length !== FIXED_SE_COUNT + 3)
  ) {
    return rejectProfile(
      "audio.profile.invalid-shape",
      "The profile requires one session BGM, one result BGM, the complete fixed SE inventory and at most one Live-start voice.",
    );
  }
  if (
  ...
  let resultBgm: AudioResultBgmResourceProfile | null = null;
  ...
  for (const resource of profile.resources) {
    ...
    if (resource?.role === "result-bgm") {
      if (resultBgm !== null) return rejectProfile("audio.profile.duplicate-result-bgm", "Result owns one prepared BGM.");
      const validated = validateResultBgmResource(resource);
      if (validated.status !== "accepted") return validated;
      resultBgm = validated.value;
      continue;
    }
  ...
  if (bgm === null || resultBgm === null || seenSe.size !== FIXED_SE_COUNT) {
    return rejectProfile(
      "audio.profile.incomplete-session-inventory",
      "The profile must contain the session BGM, result BGM and every selected SE once; defaults and substitutions are forbidden.",
    );
  }
  if (bgm.cue === resultBgm.cue || bgm.logicalId === resultBgm.logicalId ||
    (voice !== null && (voice.cue === resultBgm.cue || voice.logicalId === resultBgm.logicalId)) || seenSe.has(bgm.cue) || FIXED_SE_LOGICAL_IDS_SET.has(bgm.logicalId) ||
...
export function validateAudioCommandShape(
  ...
  switch (command.kind) {
    ...
    case "bgm.start-result":
      return hasExactKeys(command, ["kind", "cue", "fade_seconds"]) &&
        command.cue === "after_live" && command.fade_seconds === resultMedia.bgm.fadeInSeconds
        ? audioAccepted(undefined)
        : rejectCommand("audio.command.invalid-result-bgm", "Result BGM uses after_live and the original 0.25-second fade.");
    case "bgm.load":
    ...
    case "se.start-owned-loop":
      ...
      ]) && (command.cue === "SE_RHYTHM_GAYA" || command.cue === "SE_RESULT_SCORE_COUNT") && seCueSet.has(command.cue) &&
        isNonEmpty(command.owner_key) && command.volume_bits === "0x3F800000" &&
        command.fade_in_bits === (command.cue === "SE_RHYTHM_GAYA" ? "0x3F000000" : "0x00000000")
        ? audioAccepted(undefined)
        : rejectCommand(
            "audio.command.invalid-startup-loop-start",
            "Owned SE loops require a stable owner and unit volume: startup Gaya fades in over 0.5 seconds; result counting starts immediately.",
          );
    case "se.fade-owned-loop":
      return hasExactKeys(command, [
        ...
        (command.duration_bits === "0x3FC00000" || command.duration_bits === "0x00000000") && command.stop_at_zero === true
        ? audioAccepted(undefined)
        : rejectCommand(
            "audio.command.invalid-startup-loop-fade",
            "Owned loop stopping uses the cue-specific duration: 1.5 seconds for Gaya and immediate stop for result counting.",
          );
...
function validateResultBgmResource(resource: AudioResultBgmResourceProfile): AudioOperationResult<AudioResultBgmResourceProfile> {
  const m = resultMedia.bgm;
  // Decode resampling and conversion back to source frames can round by one sample.
  // The original CRI loop endpoints remain unchanged.
  if (!isRecord(resource) || !hasExactKeys(resource, RESOURCE_KEYS) || resource.logicalId !== "sound/result" || resource.cue !== m.cue ||
    !Number.isSafeInteger(resource.byteLength) || resource.byteLength <= 0 || !SHA256_PATTERN.test(resource.sha256) ||
    resource.mime !== "audio/mpeg" || resource.codec !== "mp3" || resource.sampleRate !== m.sampleRate ||
    resource.channels !== m.channels || !Number.isSafeInteger(resource.sampleFrames) || Math.abs(resource.sampleFrames - m.sampleFrames) > 1 ||
    !Number.isFinite(resource.durationSeconds) || Math.abs(resource.durationSeconds - resource.sampleFrames / resource.sampleRate) > 0.000001 ||
    !isRecord(resource.loop) || !hasExactKeys(resource.loop, ["start", "end"]) ||
    resource.loop.start !== m.loop.start || resource.loop.end !== m.loop.end ||
    resource.identity !== "semantic-exact" || resource.signal !== "portable-equivalent-lossy") {
    return rejectProfile("audio.profile.invalid-result-bgm", "Result BGM requires its prepared source identity, decoded metadata and original CRI loop range.");
  }
  return audioAccepted(Object.freeze({ ...resource, loop: Object.freeze({ ...resource.loop }) }));
}
...
function validateFixedSeResource(
  ...
  } else if (resource.cue === "SE_RHYTHM_GAYA" || resource.cue === "SE_RESULT_SCORE_COUNT") {
    if (!isRecord(resource.loop) || !hasExactKeys(resource.loop, ["start", "end"]) ||
      resource.loop.start !== 0 || resource.loop.end !== resource.sampleFrames) {
      return rejectProfile("audio.profile.invalid-loop", "Startup Gaya and result counting loop the full decoded leased source range.");
    }
  } else if (resource.loop !== null) {
    return rejectProfile(
      "audio.profile.unexpected-loop",
      "Only the evidenced Long/Slide, Gaya and result count SE resources carry loop ranges.",
    );
  }
