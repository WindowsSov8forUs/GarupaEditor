...
const ANIMATION_ROLES = new Set([
  ...
  "combo", "all-perfect", "result", "life-warning", "life-game-over",
  "score-gauge-ss",
]);
...
function validateFidelity(
  ...
  return reject(
    ...
    "HABAHIRO requires an explicit current resource format; automatic fallback is forbidden.",
  );
}
  ...
  for (const row of asset.atlasRows) {
    if (
      ...
      return reject(
        ...
        `Atlas exact keys must be unique within one logical asset and every rectangle, pivot and PPU must be finite and in bounds; asset=${asset.logicalAssetId}, key=${row.exactKey}.`,
      );
    }
    // UISprite.OnFill permits crossing inner UV edges (e.g. bg_base_tab:
    // width 81, borders 44/40). Each edge remains within the outer rectangle;
    // the host nine-slice keeps that signed center UV span.
