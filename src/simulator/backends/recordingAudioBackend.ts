...
interface MutableStartupLoop {
  readonly cue: "SE_RHYTHM_GAYA" | "SE_RESULT_SCORE_COUNT";
  paused: boolean;
}
...
export class RecordingSimulatorAudioBackend implements SimulatorAudioBackend {
  ...
  commit(batch: AudioCommandBatch): AudioOperationResult<void> {
    ...
    if (pending.commands.some(command => command.kind === "bgm.start-result")) this.bgmNaturallyEnded = false;
...
function applyCommand(
  ...
  switch (command.kind) {
    case "bgm.start-result":
      if (state.bgmCue === null || state.bgmCue === command.cue || state.allPaused || state.bgmPaused) {
        return transitionRejected("audio.command.invalid-result-handoff", "Result BGM replaces the completed running session BGM once.");
      }
    ...
    case "se.fade-owned-loop":
      if (!state.startupLoops.has(command.owner_key) || command.duration_bits !==
        (state.startupLoops.get(command.owner_key)?.cue === "SE_RHYTHM_GAYA" ? "0x3FC00000" : "0x00000000")) {
        return transitionRejected(
