import { validExponentialTransform, validExponentialLine, copyExponentialTransform, copyExponentialLine } from "../engine/rendering/exponentialCoordinates";
import {
  integrityFailure,
  ok,
...
const RENDER_OBJECT_ROLES = new Set([
  ...
  "hud-result", "hud-life", "hud-add-score", "hud-game-clear", "habahiro-root-effect", "habahiro-flash-mesh",
]);
...
export class RecordingSimulatorRendererBackend implements SimulatorRendererBackend {
  ...
  private validateCommand(
    ...
    switch (command.kind) {
      ...
      case "bind-resource": {
        ...
        if (!validateTypedRenderResourceBinding(
          command,
      ...
      case "set-transform":
        if (
          (command.unclipped?.exponential !== undefined && !validExponentialTransform(command.unclipped.exponential)) ||
          (command.unclipped !== undefined && ![command.unclipped.x, command.unclipped.y,
            command.unclipped.scaleX, command.unclipped.scaleY].every(Number.isFinite)) ||
      ...
      case "set-line":
        if (
          (command.unclipped?.exponential !== undefined && !validExponentialLine(command.unclipped.exponential)) ||
          (command.unclipped !== undefined && (!([...command.unclipped.start, ...command.unclipped.end,
            command.unclipped.width].every(Number.isFinite)) || command.unclipped.width <= 0)) ||
      ...
      case "set-hud": {
        const object = objects.get(command.renderObjectId);
        if (object === undefined || !validateTypedRenderHudCommand(command, object.role)) {
          return this.latchFault(
  ...
  private hasAnimationRole(role: RenderAnimationRole): boolean {
    ...
    if (visible !== undefined) {
      ...
      if (role === "result" || role === "life-warning" || role === "life-game-over") return true;
    }
    return false;
  }
...
function freezeCommand(command: RenderCommand): RenderCommand {
  switch (command.kind) {
    case "set-transform":
      return Object.freeze({
        ...command,
        unclipped: command.unclipped === undefined ? undefined : Object.freeze({ ...command.unclipped,
    ...
    case "set-hud":
      return Object.freeze({ ...command, state: freezeTypedHudState(command.state) }) as RenderCommand;
    case "sample-animation":
