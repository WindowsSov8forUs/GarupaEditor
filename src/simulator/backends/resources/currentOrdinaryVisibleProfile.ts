...
export function parseCurrentOrdinaryVisibleProfile(value: unknown): OrdinaryVisibleProfile | null {
  if (!record(value) || value.schemaVersion !== 1 ||
    ...
    !validateResult(value.result) || !validateLife(value.life)
  ) return null;
  if (value.sample !== undefined && (!record(value.sample) ||
    value.sample.package !== "jp.co.craftegg.band" || value.sample.versionName !== "10.1.4" ||
    value.sample.versionCode !== 230 || value.sample.abi !== "arm64-v8a")) return null;
  return deepFreeze({
    schemaVersion: 1,
    noteAnimations: value.noteAnimations,
    combo: value.combo,
    addScore: value.addScore,
    result: value.result,
    life: value.life,
  }) as unknown as OrdinaryVisibleProfile;
}
