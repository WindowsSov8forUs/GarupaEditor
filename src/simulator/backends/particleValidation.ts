...
function isParticleInstanceIdentity(value: unknown): value is ParticleInstanceIdentity {
  ...
  if (value.kind === "game-play-button") {
    return hasExactKeys(value, ["kind", "buttonType", "isHalfButton", "rangeLength", "ownerTransform", "particleSystemSetupScaleBits", "particleSystemSetupScaleFactorsBits"]) &&
      typeof value.isHalfButton === "boolean" && typeof value.buttonType === "number" &&
      Number.isInteger(value.buttonType) && value.buttonType >= 0 && value.buttonType <= (value.isHalfButton ? 5 : 6) &&
  ...
  if (value.kind !== "note-slide" ||
    !hasExactKeys(value, [
      ...
      "particleSystemSetupScaleBits", "particleSystemSetupScaleFactorsBits", "poolSlot",
    ]) ||
    typeof value.noteIndex !== "number" || !Number.isSafeInteger(value.noteIndex) || value.noteIndex < 0 ||
    typeof value.absolutePosition !== "number" || !Number.isFinite(value.absolutePosition) || value.absolutePosition < 0 ||
    ...
    !Array.isArray(value.particleSystemSetupScaleFactorsBits) || value.particleSystemSetupScaleFactorsBits.length !== 2 ||
    !value.particleSystemSetupScaleFactorsBits.every(positiveFloat32Bits) ||
    !isOwnerTransform(value.ownerTransform, "note-slide")) return false;
  return true;
}
