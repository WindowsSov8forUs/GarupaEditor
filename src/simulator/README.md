...
当前阶段为“资源与Pixi渲染”，任务书见`tmp/simulator-resource-pixi-rendering-task.md`，验收记录见`tmp/simulator-resource-pixi-rendering-acceptance.md`。Reverse `4b4ebdfada2c2deea7cb9b6d838e61b1e3240876`与Garupa 807项冻结包锁定ordinary R1 87,364 events、geometry R2 87,037 events/636 frames、visible HUD R3 19,888 events/631 frames/22 setters、Note family R4 118,152 events/1,258 aggregate frames/30 owners/6个新增Slide slices、7个1600×720实体frame，以及Long child、projection、resource/HUD/animation profile。Production已消费R4授权的front Flick/Directional exact key与sorting、标准非virtual Slide visible/invisible child及22/60 segment chain、MultipleDirectional连续组与left/right material back-line（X排序、完整XYZ、0.75f宽度、shared teardown）；Pixi支持独立Multiple line role。当前production矩阵为17 closed/current-subset、16 partial、7 blocked，clean已推送`674b1d1`的14-stage总入口通过，但RP14仍不通过。Long-after Flick、NoteSlide root Wait runtime、add-Long/add-Slide/after Multiple、advanced/threshold、完整field/HUD与production chart replay继续失败关闭；原始HAB UnityFS、natural HAB R1与original frame保持`habahiro_exact_parity_gate=open-not-claimed`。禁止把新证据、失败关闭或绿色回归解释为完整阶段/parity，禁止静默fallback、production/test联网与资源二进制入库。
