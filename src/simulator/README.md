...
用户随后要求HABAHIRO不能因缺少UnityFS、natural runtime或original frame而停留在功能缺失状态。当前新增专项任务书`tmp/simulator-habahiro-approximation-task.md`：以10.1.4静态证据为主、已有动态证据与可审计推导为辅，允许在显式选择后从Bestdori固定URL获取并哈希校验缺失资源，完整实现宽Note、Long/Slide mesh、line、field/judge、lane-change和全谱replay。运行时必须持续显示`Approximate HABAHIRO`并暴露machine-readable fidelity；原始HAB UnityFS、natural HAB R1与original frame仍作为parity差异保留，但不再作为“不实现”的理由。
