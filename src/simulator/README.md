...
Hold audio consumes manual touch ownership: Long starts at Began and fades on release or tail judgement; Slide starts/restarts at accepted Began and fades on release, visible-child miss or root end. Extended Slide retains the same chain owner across intermediate nodes. Auto judgements only select one-shot cues. Source: Reverse `audio-runtime-contract-10-1-4/audio_static_contract.json` and its exported Long/Slide Began, Ended, timeout and deactivate callers. MoveTime suppresses new touch loops, and disposal clears pending events. WebAudio frees a fading hold's logical key immediately while retaining its old source through the native 0.3-second fade, including pause/resume; its ended callback cannot delete a newer owner. Native-table comparison covers 385 judgement cue routes and hold parameters; direct command-consumer inspection covers fade/restart/pause/resume/cleanup with an in-memory host stub, not physical sound equivalence.
...
The [current application-scope dispositions](./rendering-consumption-contract.md#current-application-scope-dispositions) collect the source-rule and production-consumption conclusions. They supersede older blanket OPEN lists for excluded engine internals. Public observational labels remain conservative broader evidence claims; they do not require Unity replication. Physical window resizing preserves the running session under the [canvas-fit host policy](./adaptive-layout-contract.md#surface-revision-disposition).

`closed-native-algorithm-equivalent` means that the current 10.1.4 source state transition, calculation rule, random ownership, source resource relation and primitive output semantics are closed for the named scope. Host math remains in use. It does **not** claim instruction-exact floating results, Unity/GPU driver raster, fixed-device pixels, CRI/USM or physical speaker equivalence.
...
| Ordinary command scene and current particles | `observational-gap` | Application geometry, bounds and shared-order dispositions are listed in the rendering contract; native allocation-dependent ties and device rendering are not claimed |
...
| Score/ordinary HUD presentation | `observational-gap` | Authored layout, clip and corrected animation clocks use host font measurement/raster; score input remains product CS-V1 |
| Original Live settings and current Skin switching | `observational-gap` / `closed-static-portable` | Source package selection and identity reuse feed the current application consumers; no dynamic native renderer mutation or device-output claim |
...
| Base clear / FC / AP | `observational-gap` for rendering | Source wait, clip-finished event, separate animation clocks and background exit are implemented; canvas fitting preserves the running presentation; device rendering is not claimed |
