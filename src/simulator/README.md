...
`src/simulator`于2026-08-29先按用户fresh Windows release观察撤回提交`110f6a25`的七项production portable可见关闭结论，并依据已verify/commit/push的Reverse `d4ab8146c1e13f9cfd293dab570677607e99221a`修正对象图与生命周期。2026-08-30再次以用户相同的`npm.cmd run desktop:dev -- --release`路径复验后确认：提交`3345f9aa`确实被Vite和release host加载，但实际画面没有实质关闭用户报告；此前Browser错误variant和Windows对象存在性aggregate不满足独立原作同状态可见oracle，七项production visible结论再次撤回。Reverse `50205bc49f923551f05ea9aeb5d12e0c6898b076`现已补齐普通粒子、Slide Flash/TapKeep、rank 5与base-clear的独立原作设备像素及真实consumer身份；产品尚未按这些新锚点重做和复验。现有实现仅保留portable候选。Reverse合同明确`garupa_product_equivalence_authorized=false`；原作Unity/GPU、natural Manual FC/AP设备帧和fixed-device exact继续不声明。原作自然Auto固定status 1/base-only；GarupaEditor Auto Live只由独立产品语义进入AP presentation，不能作为原作终局证据。
