...
`src/simulator`已于2026-08-29依据已推送Reverse `e5a15b823193f09e8262d4152e2e8ec7da2af9b7`重新关闭七项可见生命周期：ordinary/game-clear粒子统一按decoded top-left raster消费，straight-alpha纹理在Linear shader与`SRC_ALPHA, ONE`组合中只应用一次particle alpha；Slide Flash从首个current after-node绑定资源，TapKeep保持稳定owner并随current迁移；`>SS` UIPanel按binary32动态扩展，`ScoreGaugeSS`进入rank 5后持续3秒循环且同rank更新不重启；Full Combo/All Perfect分别完整消费104/129个AnimationClip channel，2.2833333秒clip结束后保持最终帧到3.233秒scene exit。独立fixture、actual Pixi、真实WebGL非单位alpha framebuffer、七个串行3-fresh WebView2 runner及Windows 2400×1350自然Auto AP终局均通过；只声明Browser/Pixi portable evidence equivalence，不升级Unity/GPU fixed-device exact。
...
| Complete ordinary HUD | `closed-evidence-equivalent` | Score/Combo/Judge/Life/Auto/Pause严格事实不变；`>SS`动态panel与3秒持续SS loop由五个Float32行、7.5秒Browser和Windows跨两周期帧关闭；FC/AP 104/129 channels逐项分类消费并在clip结束后保持最终帧到scene exit |
| Ordinary particle visible composition | `closed-evidence-equivalent` | 120-system/17-root/Float32 Mesh路径在共享Lane mask和正确Backdrop上重新通过actual Pixi、fresh Browser及Windows全谱；ordinary/game-clear统一top-left UV，straight-alpha RGB不乘particle alpha，真实非单位alpha framebuffer拒绝二次alpha；native random/GPU exact继续排除 |
...
- `webview2-decode-raster/audio-graph`：以下digest读取实际WebGL framebuffer，只作锁定browser回归，不是布局authority。七项纠错后的ordinary 27 captures×3为`f3538938…d0b5`，Browser Score HUD/alpha门为`ad238b01…990d`；OLS 1-owner/13-consumer及Backdrop保留`b5abc6ad…63a58`；Skin default/limited3 `c3921b02…9439`/`0124af99…7f2f`；Garupa为`6e807e1e…0de6`；Startup视觉`5ffd7d40…f3edc`、音频`7d537afa…6adc`；MV media `34c34580…3db9`、raster `66c76856…8ddb`。cleanup后Blob/video/Pixi归零；不泛化CRI/USM、Android、speaker或Unity/GPU exact。
- `framebuffer/device-exact`：锁定设备调查已形成四项客观环境阻断，exact继续不声明。

Synthetic decoder、资源hash或typed command仍不能单独升级为真实WebView2 raster证明。

Score HUD继续使用内部CS-V1 `ruleSetId + totalScoringUnitCount`，但可见consumer已重新关闭：serialized `BEBEBE/FF3B72`先转Linear、AddScore消费source-keyed clip、五个Rank位置逐运行公式覆盖、`>SS` panel继续按current indicator扩展，SS只在进入Rank时启动并持续3秒loop且同rank score update不重启。旧primitive/digest仍不单独授权；当前结论来自Reverse `1bff69eb`与`e5a15b82`独立预言机、actual primitive、fresh Browser和Windows生产帧共同通过。
...
`mainProgramIntegrationAuthorization=true`；七项视觉生命周期专项状态为`closed-portable-release-candidate`，本地任务书为`tmp/simulator-seven-visual-lifecycle-reconfirmation-task.md`。其他能力与开放项继续以本README表格和对应专项任务书为准。
