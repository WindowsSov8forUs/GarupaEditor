...
`src/simulator`于2026-08-29先按用户fresh Windows release观察撤回提交`110f6a25`的七项production portable可见关闭结论，再依据已verify/commit/push的Reverse `d4ab8146c1e13f9cfd293dab570677607e99221a`从完整对象图与生命周期重建。当前只关闭GarupaEditor产品对该portable要求的回归：粒子消费完整atlas/material并逐粒子选UV；Slide Flash按root/front span一次选laneSize key且与TapKeep共同跟随稳定NoteSlide root；`>SS` panel动态跟随indicator、ScoreGaugeSS常态3秒循环；FC/AP终帧由scene持有，104/129 channel与真实serialized ParticleSystem owner逐phase消费。Reverse合同仍明确`production_consumption_equivalence_authorization=false`；原作Unity/GPU、natural Manual FC/AP设备帧和fixed-device exact继续不声明。原作自然Auto固定status 1/base-only；GarupaEditor Auto Live只由独立产品语义进入AP presentation，不能作为原作终局证据。
...
| Ordinary Note Pixi scene | `closed-portable-product-regression` | Reverse `d4ab8146`纠正Flash为root/front span一次选key；HABAHIRO消费multi-lane exact key，skin00只走实际拥有的single-lane root/front key。完整Garupa chart→timeline→OneFrame→particle/runtime门证明Flash/TapKeep稳定owner跨current移动、资源不重绑、不重建，terminal Stop→Clear→deactivate；不声明Unity raster等价 |
| Complete ordinary HUD | `closed-portable-product-regression` | `>SS` panel继续跟随current indicator而非冻结SS marker；ScoreGaugeSS进入rank 5只启动一次并跨7.5秒保持3秒loop；FC/AP additional clip终帧保持至base-clear callback/scene exit，逐phase实际Framebuffer和early-hide反例通过；natural Manual设备帧仍开放 |
| Ordinary particle visible composition | `closed-portable-product-regression` | actual production ParticleSystem owner消费完整atlas source、top-left per-particle UV、renderer world size、straight-alpha与一次父链Transform；同状态WebGL同时拒绝wrong-row、twice-alpha、atlas-pixels-as-world-geometry和TapKeep double-scale错误variant；native random/GPU exact不声明 |
...
`mainProgramIntegrationAuthorization=true`不变；七项视觉生命周期专项状态为`closed-portable-product-regression`，本地任务书与逐项验收为`tmp/simulator-seven-visual-fresh-user-reconfirmation-task.md`及`tmp/simulator-seven-visual-same-state-gap-matrix.md`。Unity/GPU production equivalence、natural Manual FC/AP设备帧和fixed-device exact继续`open_not_claimed`。其他能力与开放项继续以本README表格和对应专项任务书为准。
