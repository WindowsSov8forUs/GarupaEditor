...
| Complete startup direction/audio | `closed-portable` | Reverse `78e6a70e`的SD01–SD16、`b17e64e9`完整调用图及`d408d758`的SDN01–SDN04：standard Live启动Gaya，Practice不创建；Public不再携带SD/voice字段，production内部直接固定非null空角色集合与缺SoundResource路径，stage和固定时序不变；BGM prepared-paused、Retry/MoveTime及cleanup均已验收 |
...
| Original Skin settings / switching | `open-evidence-required` | Reverse `31a8ef4e`已授权`closed-static-portable`实现；Schema 11已冻结并解析original aggregate设置，且在backend创建前发布完整Skin component static-store identity inventory；selected render/audio/particle/background bytes接线及验收完成前当前能力行仍保持开放；Collabo 36必须包级失败关闭 |
...
Reverse多比例布局提交`9167dce77d0472a000b509f993b0e66e44e4797f`关闭任意初始横屏surface、StarUI/safe-area、GameCamera、gameplay/particle、UIRoot/MoveTime及MV widget参数化合同，并证据化拒绝局中revision。Reverse启动空资源修订提交`d408d758f39873c2c997107903300e58d56c59c6`证明非null空SD集合与缺语音SoundResource路径；Schema 10将这两个恒定事实完全收回simulator内部，不再要求caller提交无信息null字段。Reverse普通渲染静态/HUD提交`6908ddfa`和最终账本`b5fb3dca`继续约束未改变的原作表现；其Garupa target `2b758eb...`仅作为历史基线。CS-V1变更由tracked产品规范、独立公式测试、full-chart Auto和当前WebView2观察约束。HAB original、fixed-device exact、standalone MVView/Star3D/其他excluded和Stage 9边界不因产品Score或gameplay MV portable升级。
