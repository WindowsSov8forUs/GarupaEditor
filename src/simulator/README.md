...
The [current application-scope dispositions](./rendering-consumption-contract.md#current-application-scope-dispositions) collect the source-rule and production-consumption conclusions. They supersede older blanket OPEN lists for excluded engine internals. Public observational labels remain conservative broader evidence claims; they do not require Unity replication. The separate surface-rebuild terminal-state defect remains unresolved and prevents an all-host-features completion claim.
...
| Base clear / FC / AP | `observational-gap` for rendering | Source wait, clip-finished event, separate animation clocks and background exit are implemented; device rendering and surface-rebuild continuity are not claimed |
