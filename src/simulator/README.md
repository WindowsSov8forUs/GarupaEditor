...
HABAHIRO可审计近似专项HR01–HR12已完成，任务书见`tmp/simulator-habahiro-approximation-task.md`，验收记录见`tmp/simulator-habahiro-approximation-acceptance.md`。Production在显式`approximate-current-external`选择后从Bestdori固定allowlist准备并校验11项payload，解析179个source Sprite row，完整消费宽Note、Long/Slide 42/120 mesh、sync/Multiple line、field/judge/mask及engine-clock `flash-start → change-lane → complete`；运行时持续显示`Approximate HABAHIRO`。固定全谱oracle为371 batches、6,130 frames、217,604 commands、digest `f1e1aac4b8c9b4de6d6cefde1f04f6a69636adedde9b352c559671098f22767c`，legacy degraded与ordinary digest保持不变；已推送`60e5bf5` detached clean worktree的专项总入口和14-stage全回归均通过。HA-D01–HA-D12持续披露；原始HAB UnityFS、natural HAB R1、Root_effect原clip和original frame仅作为`open-not-claimed` parity边界，不再代表功能缺失。
