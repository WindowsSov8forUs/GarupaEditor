...
总重验aggregate gate已在R12有界关闭。完整启动音频调用图已基于Reverse `b17e64e98423bed3718ac2e76a43cde5c451ee1f`重新遍历并关闭：44个current ARM64方法、10条observation-only R1、四模式谓词、Gaya字节/loop、BGM prepared-paused/play、voice release及Retry/MoveTime/cleanup均进入机器可验证合同，四项closure计数为0。该关闭不升级Unity framebuffer、fixed-device exact、HAB original、excluded玩法或Stage 9。
...
| Complete startup direction/audio | `closed-portable` | Reverse `78e6a70e`的SD01–SD16与`b17e64e9`完整调用图：Live Manual/Auto启动Gaya全buffer loop（1.0、0.5秒fade-in），Practice两模式不创建；BGM先load并paused，Live可选voice ended后release，music edge将Gaya从current gain用1.5秒stop-at-zero并resume BGM；Retry fresh与MoveTime bypass、abort/fault/dispose已验收 |
...
- `webview2-decode-raster/audio-graph`：真实production Pixi decoders在WebView2执行`createImageBitmap`、`FontFace` glyph与WebGL raster；普通完整场景3 fresh×21 digest为`a09166cf…2199`，启动视觉3 fresh×4模式×7阶段digest保持`0f33657e…14d`。同一startup harness以production `BrowserAudioResourcePreflightAdapter`真实解码hash锁定Gaya MP3并运行WebAudio owned-loop/fade/prepared-BGM graph，独立audio digest为`88a2a310…c268`。两类digest只限定当前portable环境，不泛化跨browser/GPU、speaker或原Unity exact。
...
日常开发默认运行`npm.cmd run simulator:test`（或`simulator:test:quick`）：测试树只编译一次，以11个development groups覆盖27项compiled unit/contract（含startup callgraph）与5项static、Garupa direct chart、chart parsing、clock与Auto，不执行耗时full-chart actual-Pixi/particle/HAB或WebView2，因此不能作为release证据。发布级入口仍为`npm.cmd run simulator:test:total-revalidation`；当前32-semantic-leaf DAG包含独立startup-audio callgraph leaf、3-fresh ordinary WebView2双decoder及3-fresh startup视觉+真实Gaya WebAudio门。重复运行复用ignored Cargo target，`simulator:test:total-revalidation:clean`可强制冷编译。完整说明见[`testing/README.md`](./testing/README.md)。两级均不运行Vite/Tauri，也不声称原作framebuffer或物理输出等价。
