...
当前阶段为“资源与Pixi渲染”，任务书见`tmp/simulator-resource-pixi-rendering-task.md`。Reverse `e06940fb8f1e0519e49db3951cf5ed587e6edc2c`已关闭`ordinary-exact-habahiro-degraded`交付profile并补充ordinary geometry R2：natural Live冻结87,364个连续匿名R1事件、87,037个setter R2事件与7个1600×720实体frame；HAB portable资源锁定12项current external asset与179个Sprite row/hash，必须由host本地提供并显示`Approximate HABAHIRO`。RP03 typed合同、Note基础生命周期、基础HUD原子Reflect、本地provider/SHA/PNG预检均已完成；RP10/RP11正在建立Pixi v8后端，当前已闭合本地bytes decode、Texture/subtexture cache、Sprite bind、基础transform/ordering、release、Pixi对象allocation预留与terminal renderer显式host cleanup；R2现已提供mesh topology/vertices/UV/color/threshold、line endpoint/equal width与field/mesh transform逐字段证据；Pixi已映射22-vertex base NoteMesh，producer与threshold shader仍待实施，current sync-line textured-quad已映射portable XY语义（camera/pixel projection仍待证据），mask/HUD可见布局、animation及backend commit异常回滚继续失败关闭。原始HAB UnityFS、natural HAB R1与original frame仍保持`habahiro_exact_parity_gate=open-not-claimed`；禁止静默fallback、production/test联网、资源二进制入库及原作parity宣称。
