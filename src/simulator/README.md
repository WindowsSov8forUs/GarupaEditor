...
`src/simulator`于2026-08-29 fresh Windows release窗口复查后撤回“七项可见问题已关闭”的结论。Reverse `9c627f1f7d67491d637d6780da8da27357d86dbe`保留粒子UV/混合、Slide Flash/TapKeep、`>SS` panel、ScoreGaugeSS loop及FC/AP 104/129 channel等portable重建需求，但明确`production_consumption_equivalence_authorization=false`：既有对象数、命令、generation、自产digest、非零像素和无同状态原作对照的Windows帧不能证明用户可见修复。另已纠正自然Auto Live终局只能是base clear status 1；此前Windows“Auto AP”验收属于错误产品分支，现已撤回。七项产品可见状态继续open，直到fresh production identity、同状态独立framebuffer oracle和错误variant反例同时通过。
...
| Ordinary Note Pixi scene | `portable-requirements-retained` / `product-visible-open` | Reverse `c5223b25`逐tick Note/Slide geometry与Lane mask事实继续有效；但Slide Flash/TapKeep fresh产品帧没有同状态独立像素反例，不能由actual Pixi对象或Windows全谱观察升级为可见等价 |
| Complete ordinary HUD | `portable-requirements-retained` / `product-visible-open` | Score/Combo/Judge/Life/Auto/Pause原作事实仍可用；`>SS`、SS loop和FC/AP 104/129 channel的结构/采样门保留，但fresh产品窗口没有同状态独立像素反例，不能继续称可见等价；自然Auto只允许base clear status 1 |
| Ordinary particle visible composition | `portable-requirements-retained` / `product-visible-open` | top-left UV、straight-alpha与一次alpha primitive要求保留，WebGL错误variant门只覆盖受控primitive；ordinary产品帧的native random/GPU差异及同状态原作对照未关闭，不能由对象数、非零像素或digest升级 |
...
- `webview2-decode-raster/audio-graph`：以下digest读取实际WebGL framebuffer，只作锁定browser回归，不是布局authority。撤回可见闭环并纠正natural Auto base-only后的ordinary 27 captures×3为`68bb6eca…2d7`，该digest只锁定browser回归，不恢复原作可见等价；历史Browser Score HUD/alpha门为`ad238b01…990d`；OLS 1-owner/13-consumer及Backdrop保留`b5abc6ad…63a58`；Skin default/limited3 `c3921b02…9439`/`0124af99…7f2f`；Garupa为`6e807e1e…0de6`；Startup视觉`5ffd7d40…f3edc`、音频`7d537afa…6adc`；MV media `34c34580…3db9`、raster `66c76856…8ddb`。cleanup后Blob/video/Pixi归零；不泛化CRI/USM、Android、speaker或Unity/GPU exact。
- `framebuffer/device-exact`：锁定设备调查已形成四项客观环境阻断，exact继续不声明。

Synthetic decoder、资源hash或typed command仍不能单独升级为真实WebView2 raster证明。

Score HUD继续使用内部CS-V1 `ruleSetId + totalScoringUnitCount`；serialized `BEBEBE/FF3B72`、AddScore source-keyed clip、Rank公式、`>SS`动态panel和3秒SS loop仍是portable实现要求。但Reverse `9c627f1f`已确认既有actual primitive、Browser和Windows帧没有构成同状态四向消费门，因此Score HUD产品可见等价恢复为open，不再从Float32行、generation或digest推导关闭。
...
`mainProgramIntegrationAuthorization=true`不变；七项视觉生命周期专项状态已纠正为`portable-requirements-retained-product-visible-open`，本地任务书为`tmp/simulator-seven-visual-lifecycle-reconfirmation-task.md`。其他能力与开放项继续以本README表格和对应专项任务书为准。
