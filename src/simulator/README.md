...
当前阶段为“资源与Pixi渲染”，任务书见`tmp/simulator-resource-pixi-rendering-task.md`。Reverse `d88188f5fcea2ade712577f1c64e44002fe77d48`已关闭全部可离线10.1.4 method/resource/scene/animation/portable工作并分类H01–H28、D01–D18、PR01–PR40。HABAHIRO自然Live当前不可达：exact parity继续保持open；经用户明确授权，`historical-atlas-proxy`与`current-ordinary-stretch-proxy`组成显式degraded交付轨，强制显示`Approximate HABAHIRO`并保留HA-D01–HA-D12，禁止静默fallback和parity宣称。ordinary natural Live R1/frame及剩余contracts仍令`rendering_gate=open`、`production_authorization=false`；当前仍禁止修改模拟器production、增加Pixi backend、package scripts或资源二进制。
