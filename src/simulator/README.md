...
当前阶段为“资源与Pixi渲染”，任务书见`tmp/simulator-resource-pixi-rendering-task.md`。Reverse `e2e66f7a15b532600a3fc53f392a4c0fa2493f22`已关闭全部可离线10.1.4 method/resource/scene/animation/portable工作并分类H01–H28、D01–D18、PR01–PR40，同时锁定55个hook target、2个R1场景与13个frame anchor；`offline_work_gate=closed`、`offline_plan_gate=closed`，但当前HABAHIRO bundle、自然ordinary/HABAHIRO Live R1及实体frame必须连接游戏服务器，故`rendering_gate=open`、`production_authorization=false`。RP02关闭前禁止修改模拟器production、增加Pixi backend、package scripts或资源二进制。
