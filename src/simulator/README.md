...
原作Live设置另以Reverse `aae7e4fe`、Primary MoveNext纠正`50bc40b6`及完整HUD/AP `07bf4e7c`关闭：Public破坏性升级为Schema 13，恢复Primary/Secondary判定调整、SyncLine、NoteColor、VisibleTapLaneEffect、AllPerfectStatusDisplayMode与MvDarkness。逐字段行为、资源、产品投影和排除项见[`original-live-settings-contract.md`](./original-live-settings-contract.md)。
...
| Original Live settings | `closed-portable` | Reverse `aae7e4fe`/`50bc40b6`/`07bf4e7c`：Schema 13 Primary A、Secondary B、SyncLine、NoteColor、VisibleTapLaneEffect、AllPerfectStatusDisplayMode、MvDarkness均有冻结identity和engine/render/movie consumer；AP原作新账户默认未声明，应用按`app.simulator.all-perfect-status-display-default-on-v1`为新设置及旧缓存补`true`并继续传递显式布尔值；Retry/MoveTime/Pause lifecycle、4 PNG/13-slot owner、Actual Pixi和3-fresh WebView2；完整边界见[`original-live-settings-contract.md`](./original-live-settings-contract.md) |
