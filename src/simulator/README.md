...
`src/simulator`于2026-08-29完成七项视觉纠错的production portable消费回归：Reverse `9c627f1f7d67491d637d6780da8da27357d86dbe`继续提供粒子UV/混合、Slide Flash/TapKeep、`>SS` panel、ScoreGaugeSS loop及FC/AP 104/129 channel要求，并继续明确`production_consumption_equivalence_authorization=false`。当前实现以fresh production identity、独立Reverse fixture、完整同状态tuple和actual WebGL错误variant门关闭GarupaEditor可见回归，但不升级为原作Unity/GPU设备等价。原作自然Auto仍固定status 1/base-only；GarupaEditor自然Live Auto另由产品语义`simulator.auto-live-all-perfect-terminal-presentation-v1`进入status 3 AP演出，Record事实与Presentation明确隔离。
...
| Ordinary Note Pixi scene | `closed-portable-product-regression` | Reverse `c5223b25`逐tick Note/Slide geometry与Lane mask事实继续有效；actual WebGL固定Slide chain锁定head/first-current/current tuple、真实resource crop、稳定Flash/TapKeep owner及terminal释放，并拒绝wrong-head variant；不升级为Unity设备帧等价 |
| Complete ordinary HUD | `closed-portable-product-regression` | Score/Combo/Judge/Life/Auto/Pause原作事实仍可用；`>SS` frozen-width、SS restart-zero、FC/AP early-hide、omitted/unknown channel错误variant均经production framebuffer/profile门拒绝；产品Auto AP由独立稳定产品语义授权，原作自然Auto仍为status 1 |
| Ordinary particle visible composition | `closed-portable-product-regression` | top-left UV、straight-alpha与一次alpha进入实际production owner；game-clear authored UI/375 px-per-world-unit投影修正后与物理framebuffer相交，actual WebGL拒绝wrong-row与twice-alpha variant；native random、GPU exact与原作完整帧仍开放 |
...
状态词：`closed-portable`只表示当前证据和raw验收明确覆盖的portable合同；`closed-portable-product-regression`表示production对已授权portable合同的同状态可见消费及错误variant门已关闭，但原作production/GPU等价未授权；`closed-static-portable`表示原作选择/生命周期由静态链关闭且portable资源/backend验收完成，但不伪称非默认特殊包实机帧；`closed-product-extension`表示GarupaEditor产品合同和产品验收已闭合，但不声明原作等价；`closed-original-unreachable`表示原作合法输入不可达而非待补功能；`open-objective-environment-blocked`保持exact不声明且记录可复现环境阻断；`degraded-explicit`不等于原作parity，其余开放和排除状态按表中边界解释。
...
- `webview2-decode-raster/audio-graph`：以下digest读取实际WebGL framebuffer，只作锁定browser/product portable回归，不是Unity布局authority。七项纠错与产品Auto AP恢复后的ordinary 27 captures×3为`c33a1865…31e1`，含R01/R04/R05/R06/R07错误variant和FC/AP 8-phase/6-visible-state门；Browser Score HUD/alpha门为`ad238b01…990d`；actual WebGL固定Slide为`62bd2f11…2991`；OLS 1-owner/13-consumer及Backdrop保留`b5abc6ad…63a58`；Skin default/limited3 `c3921b02…9439`/`0124af99…7f2f`；Startup视觉`5ffd7d40…f3edc`、音频`7d537afa…6adc`；MV media `34c34580…3db9`、raster `66c76856…8ddb`。cleanup后Blob/video/Pixi归零；不泛化CRI/USM、Android、speaker或Unity/GPU exact。
...
`mainProgramIntegrationAuthorization=true`不变；七项视觉生命周期专项状态为`closed-portable-product-regression/original-production-equivalence-open`，本地任务书为`tmp/simulator-seven-visual-lifecycle-reconfirmation-task.md`。其他能力与开放项继续以本README表格和对应专项任务书为准。
