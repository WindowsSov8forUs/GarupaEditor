...
| Original Skin settings / switching | `open-evidence-required` | Reverse `977f5e71`授权`closed-static-portable`并冻结133 whole packs；Schema 11 aggregate resolver、Note/Directional/Judge/Field/Background、Tap/Judge/Directional SE、Retry/MoveTime identity及actual Pixi/WebAudio/WebView2已接线。selected special ParticleSystem模块包虽已双SHA预检，但动态模块图尚未替换fixed simulation，因此全局能力诚实保持开放；Collabo 36包级失败关闭 |
