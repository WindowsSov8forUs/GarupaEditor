...
`src/simulator`于2026-08-29再次经用户fresh Windows release窗口确认：粒子资源消费、Slide Flash/current、Slide持续特效、`>SS`粒子范围与常态高光、FC/AP显示时限及完整动效仍存在可见偏差，因此撤回提交`110f6a25`所称“七项production portable可见回归已关闭”的结论。Reverse `9c627f1f7d67491d637d6780da8da27357d86dbe`继续提供既有portable要求并明确`production_consumption_equivalence_authorization=false`；现有对象、channel、错误variant primitive与candidate framebuffer测试没有覆盖用户指出的完整对象图/生命周期。原作自然Auto仍固定status 1/base-only；开发阶段为便于可视测试，GarupaEditor Auto Live暂以独立产品语义进入AP/FC presentation，不能作为原作终局证据。
...
| Ordinary Note Pixi scene | `portable-requirements-retained` / `product-visible-open` | Reverse `c5223b25`逐tickNote/Slide事实继续有效；现有固定chain测试绕过完整runtime judgement/particle消费，未发现fresh窗口中Flash锁在段尾、resource锁定和TapKeep缺失，因此不能关闭 |
| Complete ordinary HUD | `portable-requirements-retained` / `product-visible-open` | Score/Combo/Judge/Life/Auto/Pause事实仍可用；现有mask/restart/channel framebuffer门没有回答`>SS`粒子是否只限起点→SS分界、常态高光是否持续、FC/AP是否只显示不消失及动效是否完整，七项结论撤回 |
| Ordinary particle visible composition | `portable-requirements-retained` / `product-visible-open` | top-left UV、straight-alpha primitive要求保留；fresh窗口确认资源切分大致正确但实际使用方式仍错，isolated wrong-row/twice-alpha门不能替代完整ParticleSystem renderer/object graph验收 |
...
状态词：`closed-portable`只表示当前证据和raw验收明确覆盖的portable合同；`closed-portable-product-regression`仅保留为状态词定义，当前七项不得使用该状态；`closed-static-portable`表示原作选择/生命周期由静态链关闭且portable资源/backend验收完成，但不伪称非默认特殊包实机帧；`closed-product-extension`表示GarupaEditor产品合同和产品验收已闭合，但不声明原作等价；`closed-original-unreachable`表示原作合法输入不可达而非待补功能；`open-objective-environment-blocked`保持exact不声明且记录可复现环境阻断；`degraded-explicit`不等于原作parity，其余开放和排除状态按表中边界解释。
...
`mainProgramIntegrationAuthorization=true`不变；七项视觉生命周期专项状态重新恢复为`portable-requirements-retained-product-visible-open`，本地任务书为`tmp/simulator-seven-visual-fresh-user-reconfirmation-task.md`。其他能力与开放项继续以本README表格和对应专项任务书为准。
