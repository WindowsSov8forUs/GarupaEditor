...
`src/simulator`于2026-08-29按用户fresh Windows release观察撤回提交`110f6a25`的七项production portable可见关闭结论；2026-08-30又确认`3345f9aa`虽被Vite/release host加载，但未实质关闭普通单宽判定粒子和FC/AP退场。当前纠错锁定已verify/commit/push的Reverse `e7e419bcc91433bf058d06b9548fa9729e6e49e1`：普通目标完整冻结为8 roots、56 systems、49 enabled renderers；current `libunity.so` ARM64进一步证明ShapeModule type 5将centered random XYZ交给birth position、将独立local `+Z [0,0,1]`交给direction handoff，历史portable `+Y`reference/oracle只保留字节身份而不再充当current authority。FC/AP Animator严格执行`text-in -> text-out -> alpha-zero terminal`，不返回default，也不把3.233秒base callback冒充additional文字hold。current corrected 5-case/17-root oracle、24 Box systems/12 critical transformed-velocity、B.B.K width-1 actual WebGL门和三次fresh WebView2均已通过；本次axis纠错后的真实release窗口也以2400×1350 backing、普通gameplay low/high峰值35/269 children完成同产品输入复验。原作提交帧因相邻route事件不能归因为单root/单lane，故connected-component数不跨chart/random tuple迁移。Reverse合同仍明确`garupa_product_equivalence_authorized=false`；原作Unity/GPU、native random、natural Manual FC/AP设备帧和fixed-device exact继续不声明。原作自然Auto固定status 1/base-only；GarupaEditor Auto Live只由独立产品语义进入AP presentation，不能作为原作终局证据。

此前Lane SpriteMask与舞台背景合成纠错继续有效。Reverse `dbd009e7`明确13个Lane SpriteRenderer只共享一个serialized `MaskImage` pathId 540；旧`3ae2d1a7`既按槽复制13个mask，又在同一set-transform末尾以generic null mask清空引用，使Graphics重新进入ordinary color build并把Backdrop合成为大块白色。当前PixiRendererBackend恢复一个scene-owned mask/13个同identity inverse引用，独立管理consumer与generation dispose，且禁止generic command mask覆盖。关闭依据包括生产实时Pixi树和删除/共享AB帧、actual Pixi 656/3900、OLS 3-fresh背景sentinel/white-pixel门、ordinary 3×27、45叶全量回归及Windows 2400×1350 startup→play→Pause→countdown→resume→AP→自然关闭；旧`1a08056f`及其全部产物永久撤回。10M归一化Score、固定Rank和内部ruleset继续由[`scoring-contract.md`](./scoring-contract.md)以产品身份授权，不宣称原作计分复原。
...
B.B.K用户谱的单宽普通判定粒子可见回归绑定产品语义`simulator.bbkk-single-width-ordinary-particle-visible-regression-v1`：actual WebGL完整场景必须同时发布完整Pixi粒子、左右细火花与左右大范围判定glow，且在不透明backdrop上加入additive粒子不得降低任一RGB channel。该门只防止产品再次退化成无粒子/单透明圆/黑色atlas块，不把不同`Ichininaru` chart、timeline、并发owner和native-random tuple的原作connected-component数移植为等价判定；Reverse `4564b228`已明确原作提交帧不能归因为单root/单lane，`e7e419bc`只追加授权type-5 local axis从`+Y`纠正为`+Z`，不迁移任何framebuffer threshold。Unity/GPU、native random与fixed-device逐像素等价继续开放。
...
| Ordinary Note Pixi scene | `open-product-visible-reconfirmation` | Reverse `d4ab8146`纠正Flash为root/front span一次选key；但用户实际路径为skin00、`habahiro=false`，新增HAB multi-lane key分支未被消费，窗口画面未实质变化。Flash/TapKeep结构候选不能替代同状态原作可见对照 |
| Complete ordinary HUD | `open-product-visible-reconfirmation` | 动态`>SS` panel、3秒SS loop及FC/AP scene hold是portable候选；现有Framebuffer只比较自产错误variant，Windows截图只证明对象出现，未提供独立原作同状态expected |
| Ordinary particle visible composition | `open-product-visible-reconfirmation` | atlas/UV/Transform代码与actual WebGL Mesh存在性已覆盖，但用户窗口中的ordinary/TapKeep及AP粒子构图仍未被独立原作同状态可见oracle关闭；native random/GPU exact同样不声明 |
| Gameplay/Startup Audio、Particle semantic simulation | `closed-portable` | Public BGM字节派生、已登记判定/Note SE、四模式startup callgraph、Live-only Gaya owned loop、原作nullable voice分支但production内部固定无voice资源、prepared BGM、semantic/PCM/WebAudio graph及deterministic particle command/simulation限定范围；particle current expected显式消费Reverse `e7e419bc`的type-5 `+Z` corrected oracle，历史`+Y`仅保留身份；不含native random、物理speaker输出或framebuffer exact |
...
| WebView2 decode/glyph/raster | `closed-portable` | 真实WebView2 151.0.4129 patch line执行production PNG/FontFace/Pixi WebGL；Reverse `50170414`取代旧premultiply假设，`e5a15b82`再锁定ordinary/game-clear统一top-left UV与一次alpha。受控非单位alpha actual framebuffer、错误二次alpha反例、game-clear row 2/拒绝row 1及alpha0脏RGB均通过，不泛化为Unity/GPU exact |
...
状态词：`closed-portable`只表示当前证据和raw验收明确覆盖的portable合同；`closed-portable-product-regression`表示GarupaEditor production对Reverse已授权portable要求的完整消费回归已由独立fixture、primitive流、同状态实际Framebuffer错误variant与fresh产品身份共同关闭，但不声明Unity/GPU production等价；`closed-static-portable`表示原作选择/生命周期由静态链关闭且portable资源/backend验收完成，但不伪称非默认特殊包实机帧；`closed-product-extension`表示GarupaEditor产品合同和产品验收已闭合，但不声明原作等价；`closed-original-unreachable`表示原作合法输入不可达而非待补功能；`open-objective-environment-blocked`保持exact不声明且记录可复现环境阻断；`degraded-explicit`不等于原作parity，其余开放和排除状态按表中边界解释。
...
- `webview2-decode-raster/audio-graph`：以下digest读取实际WebGL framebuffer，只作锁定browser/product portable回归，不是Unity布局authority。Reverse `d4ab8146`纠错后的ordinary 27 captures×3为`9d58b7e4…c1ed1`，另含完整runtime TapKeep稳定owner/三次root transform/terminal清理及R01 actual owner wrong-row、twice-alpha、atlas-geometry错误variant，FC/AP每phase`m_IsActive`命中真实playOnAwake ParticleSystem；Garupa Slide Flash 4 captures×3为`656da37e…83e3`；Browser Score HUD/alpha门为`ad238b01…990d`；actual WebGL固定Slide为`62bd2f11…2991`；OLS 1-owner/13-consumer及Backdrop保留`b5abc6ad…63a58`；Skin default/limited3 `0e616fa7…457`/`c480485b…55f`；Startup视觉`5ffd7d40…f3edc`、音频`7d537afa…6adc`；MV media `34c34580…3db9`、raster `66c76856…8ddb`。cleanup后Blob/video/Pixi归零；不泛化CRI/USM、Android、speaker或Unity/GPU exact。
- `framebuffer/device-exact`：锁定设备调查已形成四项客观环境阻断，exact继续不声明。

Synthetic decoder、资源hash或typed command仍不能单独升级为真实WebView2 raster证明。

Score HUD继续使用内部CS-V1 `ruleSetId + totalScoringUnitCount`；serialized `BEBEBE/FF3B72`、AddScore source-keyed clip、Rank公式、`>SS`动态panel和3秒SS loop仍是portable实现要求。当前production门不再由Float32行、generation或candidate digest自证：五个score行分别读取actual framebuffer并拒绝frozen-width，SS同phase framebuffer拒绝same-rank restart-zero；该结论仅关闭产品portable回归，不声明原作GPU等价。
