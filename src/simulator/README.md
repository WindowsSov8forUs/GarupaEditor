...
`src/simulator`于2026-08-29按用户fresh Windows release观察撤回提交`110f6a25`的七项production portable可见关闭结论；2026-08-30又确认`3345f9aa`虽被Vite/release host加载，但未实质关闭普通单宽判定粒子和FC/AP退场。当前纠错锁定已verify/commit/push的Reverse `4564b228dd5400a92429bc1cbd3ee02de2007ec4`：普通目标完整冻结为8 roots、56 systems、49 enabled renderers；FC/AP Animator严格执行`text-in -> text-out -> alpha-zero terminal`，不返回default，也不把3.233秒base callback冒充additional文字hold。B.B.K width-1 actual WebGL门、三次fresh WebView2和真实`npm.cmd run desktop:dev -- --release`窗口均已关闭产品portable/release回归；原作提交帧因相邻route事件不能归因为单root/单lane，故connected-component数不跨chart/random tuple迁移。Reverse合同仍明确`garupa_product_equivalence_authorized=false`；原作Unity/GPU、native random、natural Manual FC/AP设备帧和fixed-device exact继续不声明。原作自然Auto固定status 1/base-only；GarupaEditor Auto Live只由独立产品语义进入AP presentation，不能作为原作终局证据。
