...
`src/simulator`已完成2026-08-28 HUD/粒子/Lane/Pause/终局八域严格再审。Reverse `dbd009e7`在此前`552433d0`资源/轴向纠错之上补齐Score SS同深度兄弟序、Auto Awake owner、Combo/Judge独立消失、粒子Linear Float32色、Lane flip/mask、Pause UILabel/countdown clips及FC/AP面板；旧`329aec79`及更早声明/产物均撤回。生产实现现保持Flash→BigStar→kira使动态星光不被静态overlay盖住，Auto caption参与startup HUD alpha，Combo在1秒scale结束后保留而Judge独立隐藏，判定粒子使用共享GlProgram+per-Mesh Float32 uniform，13槽恢复右侧flipX与VisibleOutsideMask，Pause恢复serialized text boxes和25-curve倒计时，FC/AP保持base+additional、UIPanel clipping None、3.233秒endpoint与15ms exit。能力恢复来自原作节点→production consumer→独立primitive→actual Pixi/fresh Browser/Windows全谱四向门：45叶全量回归、ordinary 3×27及Windows B.B.K 676 Combo/ALL PERFECT连续帧均通过。10M归一化Score、固定Rank和内部ruleset继续由[`scoring-contract.md`](./scoring-contract.md)以产品身份授权，不宣称原作计分复原。
...
| Complete ordinary HUD | `closed-evidence-equivalent` | Score encoded sRGB色/TTF/动态Rank及SS 11节点Flash→BigStar→kira序、Life10组件、AddScore4×7 local-X、Auto Awake/hudAlpha、Combo保持与Judge独立hide、Pause text boxes/countdown曲线、base/FC/AP均完成四向消费；Windows连续SS crop显示kira位置/大小逐帧变化，AP保持到退出 |
| Ordinary particle visible composition | `closed-evidence-equivalent` | default master id1=`skin00`固定120-system/8-logical/7-unique-PNG，Flick/Directional/17 roots、嵌套g² position/velocity与Local one-g billboard size保持；sample颜色不经RGB8 tint，以共享GlProgram+per-Mesh Float32 Linear uniform进入normal/add blend。actual Pixi、default/Limited各3-fresh及Windows全谱关闭；native random/GPU exact继续排除 |
...
| Original Live settings | `closed-evidence-equivalent` | Schema 13字段、A/B、SyncLine、NoteColor、AP、MvDarkness及VisibleTapLaneEffect由冻结owner消费；13槽除serialized Transform与white/alpha Fade外，恢复slots8..12 `flipX`和全部VisibleOutsideMask；OLS 3-fresh digest `c9113024…8d60`及非零active raster关闭。完整边界见[`original-live-settings-contract.md`](./original-live-settings-contract.md) |
...
| Original in-game Pause UI | `closed-evidence-equivalent` | Retryable/Selectable/Annotated逐component Transform/widget/depth/pivot/font持久实例化，UILabel固定box/wrap；Resume不再是同一Sprite换图，而是Contents/Count3/2/1/1Fadeout/Fill持久图逐帧采样3.5秒25-curve clip并在3秒callback恢复。actual Pixi、3-fresh Browser及Windows Pause/countdown连续帧通过 |
