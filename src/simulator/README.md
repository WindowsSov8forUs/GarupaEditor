...
总重验aggregate gate已在R12有界关闭，但完整启动音频调用图因遗漏`SE_RHYTHM_GAYA`已重新开放；production engine construction现于chart construction、资源读取、backend prepare、scheduler与mount之前失败关闭。这不撤回已独立关闭的四模式identity/Life/MoveTime合同，也不升级Unity framebuffer、fixed-device exact、HAB original、excluded玩法或Stage 9。
...
| Gameplay Audio、Particle semantic simulation | `closed-portable` | Public BGM字节派生、已登记判定/Note SE、semantic/PCM/WebAudio graph及deterministic particle command/simulation限定范围；不包含当前重新开放的完整startup audio调用图，也不含物理输出/framebuffer |
...
| Complete startup direction/audio | `open-evidence-required` | Reverse `78e6a70e`的视觉、presentation、状态0→5与voice/BGM子集仍可测试，但旧closure遗漏`ExecStart → playGayaSound → PlaySELoop(SE_RHYTHM_GAYA)`，且背景虚调用、动画事件、BGM prepare/play和全部cleanup尚未逐项关闭；旧WebView2 digest仅为视觉子门 |
...
- `webview2-decode-raster`：真实production `BrowserPixiTextureDecoder`与`BrowserPixiParticleTextureDecoder`在WebView2执行`createImageBitmap`、`FontFace` glyph及Pixi WebGL raster；普通完整场景本轮3 fresh×21 digest为`a09166cf…2199`，旧启动视觉子门3 fresh×4模式×7阶段digest为`0f33657e…14d`。后者不覆盖Gaya或完整音频调用图；digest只限定当前portable环境，不泛化跨browser/GPU、speaker或原Unity exact。
