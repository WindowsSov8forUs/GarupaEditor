...
- Atomic behavior: `completeLiveAudio` preflights Game-clear particle, audio, NGUI render and tap-lane all-off, then uses one `FrameMutationPlan`. The completion flag and every Simulator owner publish only after all detached external commits succeed; physical AudioNode/GPU effects remain explicitly non-rollbackable external boundaries.
...
The Score presentation's authored 64-object/45-widget graph, ScoreRankData rules and SS/SSS animation retain their source bindings to Reverse `dddab345825dbff6d2a5cf65f5fbbcf771b00e07`. Text now uses host font measurements and width fitting; the historical CharacterInfo and Android shrinking comparisons do not establish equivalence of that path. The active session supplies `product-cs-v1`, while Public `ordinaryHud`/`selectedHudGate` remain `observational-gap`. Current presentation acceptance follows [the rendering consumption contract](rendering-consumption-contract.md), without an aggregate native-equivalence or device-raster claim.
