...
The Score presentation algorithm is separately source-bound to Reverse `dddab345825dbff6d2a5cf65f5fbbcf771b00e07`: complete 64-object/45-widget NGUI graph, native ScoreRankData rank domain, encoded-label CharacterInfo metrics, current GLES3 SoftClip and SS/SSS animation are `closed-native-algorithm-equivalent`. The active session still supplies `product-cs-v1`, so Public `ordinaryHud`/`selectedHudGate` report a product extension rather than whole original Score parity. Browser glyph raster and GPU quantization remain outside that claim.
