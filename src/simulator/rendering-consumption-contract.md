...
Slide child Move stops only after progress exceeds 1 and local Y crosses its selected line. With a waiting/stopped root, intermediate children use the real target line; other cases use VirtualPerfectLine. B==0, real-line intermediates and B>=1 terminals snap to the target X/Y while retaining Z; crossing below the real line kills the incoming segment. Reverse `de5dd893fe2af9af2fcf1a23f6d7f7174c6511a4`, `manual-input-runtime-contract-10-1-4/slide_child_stop_rules.json`, supplies 752 byte-verified instructions and the strict progress gate. The actual child-frame producer matches 48 source-rule comparisons across parent state, successor presence, adjustment sign and the progress boundary. Both TypeScript checks pass. This replaces the incorrectly shared Long stop condition; child Stop movement and miss dispatch remain separate follow-up work.

Slide position-table generation ends at the original goalY-100 threshold, not an arbitrary 512 iterations, and the consumer accepts the resulting table instead of imposing a 64-position cap. The existing `manual-input-runtime-contract-10-1-4/arm64/03223978__SlideNoteManager__setupPositionEmptyJudgeDataList.arm64.tsv` has 143 rows verified against the locked binary. Actual scene construction at speeds1/6/10/11 produces621/339/114/58 positions and successful on-time Perfect judgements. A non-advancing motion still fails explicitly. Both TypeScript checks pass; this repairs valid ordinary scene/input configurations, without adding tests or expanding supported note families.

Slide Began/rebinding and continuation share the original position judge. Its upper and lower halves select intervals from their respective outer endpoints; correction is positive before the line and negative after it, and positions outside the retained endpoints return None. Root judgement clamps virtual Y at VirtualPerfectLine; after nodes retain their own positions. Reverse `4ba9c4de99ae625c8b8e5e895c93c63802d20084`, `manual-input-runtime-contract-10-1-4/slide_position_judgement_correction.json`, executes the original setup/Judge on explicit coordinate inputs. All 65 positions and both corresponding Began paths now agree (195 comparisons). Manual head Began also publishes the source target X/Y/Z before the next note update, and binding a Flick tail hides its sprite/icon. Those outputs follow 468 verified rows from ExecTouchBegan/setVirtualPos; actual production placement matches the independently executed target-vector load and its pending hide reaches the existing renderer. Both TypeScript checks pass; no application or visual validation was used.

Manual judgement rounds frame distance to nearest with ties to even before applying the existing +3/+6/+7/+8 thresholds. Long terminal movement/release passes sweetFrame1 through the original CalculateTouchEndedJudge rule. Reverse `df1de2c2ca00d4c5039616efaa0b24c4beb2cc6f`, `manual-input-runtime-contract-10-1-4/judgement_window_correction.json`, records 525 byte-verified instructions and 32 native window cases, including four formerly divergent half-frame boundaries. Those four production boundaries and six actual Long terminal paths at three frames now agree; both TypeScript checks pass. The rounding rule is an application decision implemented with host arithmetic, not ARM approximation or Unity runtime emulation.

Stationary input invokes the same continuation path as Moved, preserving its coordinates and existing finger owner. It must still judge a Slide node arriving under a held finger and advance continuation grace. Reverse `bf9085c1a9d38795271c8586e8f45c57385e41a0`, `manual-input-runtime-contract-10-1-4/input_phase_dispatch_correction.json`, verifies the native jump table and five phase branches. The actual GamePlayButton dispatch matches all three supported continuation phases, and its stationary Slide path produces the same type8 Perfect reservation as Moved at the line. Both TypeScript checks pass. The separate Long manual-tail mapping candidate was rejected: native normal2/flick5/directional6/multiple7 already matches production.

Manual Slide terminal judgement uses the root AfterNoteType rather than the authored GameNoteType: normal8, flick5, directional6, multiple-directional7; timeout remains8. Normal Ended requires the current target lane and uses Slide position judgement/timing. Flick movement uses the Slide judge window, resets its gesture origin before VirtualPerfectLine, and accepts only after reaching that line with positive leave grace. Successful movement produces the Slide result (Perfect when adjustment B>=1); an unfinished flick released without successful movement produces Miss. Incoming flick timing is None because the bound root virtual position is clamped at VirtualPerfectLine. Reverse `49b1aceeeba6f80492f7792dfa9cf76a2b1c86a8`, `manual-input-runtime-contract-10-1-4/slide_terminal_judgement_correction.json`, verifies the original type/result branch and 1,612 additional instruction rows. Actual production methods match 234 terminal movement/release comparisons and 90 native release result/timing comparisons; the registered OneFrame allowed types and downstream consumer were checked. Both TypeScript checks pass; no test files or application/visual runs were added. This closes these terminal rules, without asserting whole-simulator equivalence.
...
The table summarizes reviewed rules and repaired differences using existing independent source outputs, consumer review and focused execution of changed methods. It does not yet establish an exhaustive mapping from current supported production branches and semantic boundaries to applicable evidence, so it cannot by itself close whole-scope algorithm equivalence or establish the absence of necessary evidence gaps. No new application-algorithm difference is asserted by this coverage limitation. Reuse of historical results requires checking affected consumers and dependencies as specified by evidence-workflow.md. Historical OPEN paragraphs must be assessed for current application relevance rather than all reopened or all dismissed. Host numerical/raster behavior and the existing excluded features remain outside this review.
...
Manual Slide release now consumes the current visible node, skips Hidden support nodes and clears the finger identity while retaining future nodes. A later Began binds the current node; it does not rejudge the head or reject merely because the head was already consumed. Flick tails bind before movement/release judgement. These rules follow the current `NoteSlide.ExecTouchEnded`, `onMiss` and `ExecTouchBegan` exports in Reverse `manual-input-runtime-contract-10-1-4` (latest source commit `fbb0fb54ce2fd714121b2745cfffbee8fb213053`). Four actual production release cases match independently executed native lifetime/finger results; Hidden cursor advancement and rebinding were checked against the source call chain. Both TypeScript checks pass. This closes that input-state defect, not the remaining ordinary Slide sprite/mesh consumption review.

The Slide visibility consumer now applies native hide requests to the body/Flick icon and successor segment. A segment is disabled when its local Y is at or below the judgement line, or the caller requests forced cleanup; later geometry updates cannot reactivate it. Input/state-phase requests sample the existing position, while forcePerfectOnUpdate requests sample the updated child position. Miss/release/Hidden cleanup and MoveTime refresh preserve their forced branch. Refresh is flushed before the reconstructed scene is published. Reverse `574beb87eeeb2dc834637a29d25337d56cf64a44`, `live-rehearsal-runtime-contract-10-1-4/ordinary_slide_visibility.json`, binds eight existing exports (2,106 verified instructions) and the 144-byte KillMesh consumer. The predicate, callback order, persistent visibility and Pixi hide-object consumption were reviewed at source level; both TypeScript checks pass. This is application visibility, not Unity Renderer emulation or pixel verification.

Auto/MoveTime Long and Slide heads now perform the subsequent `NoteFrontBase.repositionToJudgeLine` call: goal X/Y, preserved Z and motion progress, and the existing scale formula evaluated at the goal. The source is `resource-pixi-rendering-runtime-contract-10-1-4/arm64/030e05f0__NoteFrontBase__repositionToJudgeLine.arm64.tsv`, byte-verified against the locked ELF, and the Long/Slide forcePerfect callers. Four independently executed native position cases match the production helper; the producer and frame consumer are connected, and both TypeScript checks pass.
