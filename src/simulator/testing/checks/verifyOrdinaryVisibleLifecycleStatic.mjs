...
const test = readFileSync(join(root, "src/simulator/testing/cases/rendering/ordinaryVisibleLifecycleContract.test.ts"), "utf8");
...
if (rows.length !== 14 || !rows.every((row) => /^[0-9a-f]{40}$/.test(row.sourceReverseCommit))) {
...
console.log("full visible/lifecycle oracle static independence verified: fixture=14 production-helper-imports=0");
