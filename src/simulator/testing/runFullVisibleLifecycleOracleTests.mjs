...
const outputRoot = sharedOutputRoot ?? mkdtempSync(join(tmpdir(), "garupa-full-visible-lifecycle-oracle-"));
...
try {
  ...
  run(process.execPath, [join(outputRoot, "src", "simulator", "testing", "fullVisibleLifecycleOracle.test.js")]);
  run(process.execPath, [join(testingRoot, "verifyFullVisibleLifecycleOracleStatic.mjs")]);
} finally {
