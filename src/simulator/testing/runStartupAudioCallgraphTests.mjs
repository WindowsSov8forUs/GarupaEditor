...
const outputRoot = sharedOutputRoot ?? mkdtempSync(join(tmpdir(), "garupa-startup-audio-callgraph-tests-"));
...
try {
  ...
  run(process.execPath, [join(outputRoot, "src", "simulator", "testing", "startupAudioCallgraph.test.js")]);
  run(process.execPath, [join(testingRoot, "verifyAudioStatic.mjs")]);
  run(process.execPath, [join(testingRoot, "verifyStartupDirectionStatic.mjs")]);
