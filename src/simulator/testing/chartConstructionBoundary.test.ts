...
test("每次构造调用建立独立上下文并在 C06 失败关闭", () => {
  ...
  assert(first.status === "evidence-required", "C06 must remain fail-closed");
  assertEqual(first.capability, "chart-construction.long-slide-graph", "failure boundary");
  assert(
