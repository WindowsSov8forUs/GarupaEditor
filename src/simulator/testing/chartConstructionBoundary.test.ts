...
test("每次构造调用建立独立上下文并在 C07 失败关闭", () => {
  ...
  assert(first.status === "evidence-required", "C07 must remain fail-closed");
  assertEqual(first.capability, "chart-construction.multi-range-combine", "failure boundary");
  assert(
