...
} from "../../support/session/modeFixtures";
...
import { CURRENT_ORDINARY_VISIBLE_PORTABLE_RESOURCES } from "../../support/resources/currentOrdinaryVisibleTestManifest";
...
import { augmentScoreHudProfilesForPause } from "../../support/session/pauseControlResources";
...
import { observePixiWorld } from "../../support/rendering/pixiWorldObserver";
import { readWebGlFramebufferRgba } from "../../support/platform/readWebGlFramebuffer";
import {
  assertVisibleScenarioIdentity,
  type VisibleScenarioIdentity,
} from "../../support/platform/visibleScenarioIdentity";
import { applicationLeaseParticleProviderForTesting } from "../../support/resources/applicationParticleProvider";
...
interface BrowserSession {
  ...
  readonly chartIdentity: Readonly<{ readonly identity: string; readonly sha256: string | null }>;
...
interface FrameCapture {
  ...
  readonly scenarioIdentity: Readonly<VisibleScenarioIdentity>;
  readonly owners: {
...
async function captureGameClearBranchFramebuffer(
  ...
  const engineSnapshot = requireOk(session.engine.snapshot());
  const scoreLifeState = engineSnapshot.managers.scoreLifeState;
  const clearStatus = session.engine.getNaturalCompletionClearStatus();
  const world = observePixiWorld(session.combined.root);
  const visibleWorldRecordCount = world.records.filter((record) =>
  ...
  const result = Object.freeze({
    ...
    scenarioIdentity: assertVisibleScenarioIdentity({
      mode: session.mode.startsWith("rehearsal") ? "rehearsal" : "live",
      input: session.mode.endsWith("auto") ? "auto" : "manual",
      chart: session.chartIdentity,
      timeline: Object.freeze({
        frame: Math.max(0, Math.round(engineSnapshot.adjustedMusicPosition * 1_000)),
        adjustedMusicPosition: engineSnapshot.adjustedMusicPosition,
      }),
      score: Object.freeze({
        value: scoreLifeState?.record.score ?? 0,
        rank: scoreLifeState?.scoreGauge.currentGaugeColorRank ?? 4,
      }),
      owner: Object.freeze({
        sessionId: session.id,
        renderObjectCount: session.renderer.snapshot().objectCount,
        particleNodeCount: session.particleRenderer.snapshot().nodeCount,
        visibleWorldRecordCount,
      }),
      phase: `${animationKey}:${hideBranch ? "branch-hidden" : "branch-visible"}`,
      terminalStatus: clearStatus === null ? "active" : `clear-${clearStatus}`,
      viewport: Object.freeze({ width: WIDTH, height: HEIGHT }),
      dpr: Number((app.renderer as unknown as { readonly resolution?: number }).resolution ?? 1),
    }),
  });
...
async function verifyBbkkSingleWidthOrdinaryFramebuffer(
  ...
  const session = await createSession(
    inputs,
    "ordinary-webview2-bbkk-single-width",
    "live-auto",
    chart,
    Object.freeze({
      identity: "bbkk-single-width-product-regression",
      sha256: inputs.bbkkProvenance.sha256,
    }),
  );
  mount(app, session);
  ...
  for (let frame = 1; frame <= 4_000; frame += 1) {
    ...
    const additiveBackdrop = featurePassed
      ...
      : null;
    const bbkkWorld = observePixiWorld(session.combined.root);
    const bbkkVisibleWorldRecords = bbkkWorld.records.filter((record) =>
      record.visible && record.renderable && record.worldBounds !== null &&
      intersects(record.worldBounds, [WIDTH, HEIGHT])).length;
    const bbkkScore = engine.managers.scoreLifeState;
    const scenarioIdentity = assertVisibleScenarioIdentity({
      mode: "live",
      input: "auto",
      chart: session.chartIdentity,
      timeline: Object.freeze({ frame, adjustedMusicPosition: engine.adjustedMusicPosition }),
      score: Object.freeze({
        value: bbkkScore?.record.score ?? 0,
        rank: bbkkScore?.scoreGauge.currentGaugeColorRank ?? 4,
      }),
      owner: Object.freeze({
        sessionId: session.id,
        renderObjectCount: session.renderer.snapshot().objectCount,
        particleNodeCount: session.particleRenderer.snapshot().nodeCount,
        visibleWorldRecordCount: bbkkVisibleWorldRecords,
      }),
      phase: "bbkk-bilateral-particle-feature",
...
async function createSession(
  ...
  chartIdentityOverride?: Readonly<{ readonly identity: string; readonly sha256: string | null }>,
  ...
  const chartIdentity = chartIdentityOverride ?? Object.freeze({
    identity: chartOverride === undefined ? "ordinary-browser-bms" : `inline-product-chart:${id}`,
    sha256: chartOverride === undefined ? inputs.chartSha256 : null,
  });
  return {
    id, mode, chartIdentity, engine, renderer, particle, particleRenderer, combined, layout,
    controlOverlay, linearOutput, audio: visualLifecycleAudio, mounted: false,
  };
}
...
async function capture(
  ...
  const scoreLifeState = engineSnapshot.managers.scoreLifeState;
  const clearStatus = session.engine.getNaturalCompletionClearStatus();
  const terminalStatus: VisibleScenarioIdentity["terminalStatus"] = scoreLifeState?.record.singleGameOver === true
    ? "game-over"
    : clearStatus === null ? "active" : `clear-${clearStatus}`;
  const renderObjectCount = session.renderer.snapshot().objectCount;
  const particleNodeCount = session.particleRenderer.snapshot().nodeCount;
  const scenarioIdentity = assertVisibleScenarioIdentity({
    mode: session.mode.startsWith("rehearsal") ? "rehearsal" : "live",
    input: session.mode.endsWith("auto") ? "auto" : "manual",
    chart: session.chartIdentity,
    timeline: Object.freeze({ frame, adjustedMusicPosition: engineSnapshot.adjustedMusicPosition }),
    score: Object.freeze({
      value: scoreLifeState?.record.score ?? 0,
      rank: scoreLifeState?.scoreGauge.currentGaugeColorRank ?? 4,
    }),
    owner: Object.freeze({
      ...
      renderObjectCount,
      particleNodeCount,
      visibleWorldRecordCount: visibleWorldRecords,
    }),
    phase: label,
    terminalStatus,
    viewport: Object.freeze({ width: WIDTH, height: HEIGHT }),
  ...
  return Object.freeze({
    ...
    owners: Object.freeze({
      render: renderObjectCount,
      particles: particleNodeCount,
      visibleWorldRecords,
...
async function loadInputs(): Promise<LoadedInputs> {
  ...
  const chartSha256 = await sha256(new TextEncoder().encode(chartText));
  ...
  return Object.freeze({
    chartText, chartSha256, strict, sevenVisual, freshSevenVisual, focusedTerminal, bbkkChart, bbkkProvenance,
