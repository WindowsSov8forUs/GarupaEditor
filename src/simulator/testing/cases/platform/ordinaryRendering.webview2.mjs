...
try {
  ...
  run(process.execPath, [
    esbuild,
    join(testingRoot, "cases/platform/ordinaryRendering.webview2.test.ts"),
    "--bundle",
    "--platform=browser",
    "--format=iife",
    "--target=chrome120",
  ...
  console.log(
    `ordinary production WebView2 portable product observation passed (original Unity/GPU equivalence remains open): fresh=${freshRunCount} captures=${observed[0].scene.captures.length} ` +
