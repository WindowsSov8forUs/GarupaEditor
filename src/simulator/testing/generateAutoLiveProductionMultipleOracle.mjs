...
if (!compiledSimulatorRoot || !outputPath) {
  throw new Error("usage: node generateAutoLiveProductionMultipleOracle.mjs <compiled-simulator-root> <output-json>");
}
...
writeFileSync(resolve(outputPath), `${JSON.stringify({
  status: "fixed-independent-source-order-production-oracle",
  evidence: ["G21", "R16.D17"],
  generator: "src/simulator/testing/generateAutoLiveProductionMultipleOracle.mjs",
  charts,
}, null, 2)}\n`);
