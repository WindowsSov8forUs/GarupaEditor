...
- `reverse-snapshots/full-visible-lifecycle/` 包含已推送Reverse `3be484bc`的全量再审合同、14,702-event R1 trace及10张最小语义帧：Score编码色与动态Rank公式、SS只在Rank切换触发、Auto/AddScore/Life/Pause、Slide/Lane/default skin00粒子、43对象base clear、25对象Full Combo、36对象All Perfect及base44/FC104/AP129曲线；另含Reverse `767102ca`登记的B.B.K 778-item产品输入与83 Slide/141 segment全时间轴独立oracle。原作帧只作补充观察；独立primitive/lifecycle/Slide expected不读取Garupa生产helper。该旧合同的聚合production authorization已被下述第二次消费预言机撤回。
- `reverse-snapshots/second-visible-consumer/` 只包含已推送Reverse `1bff69eb`的纠错消费预言机：明确禁止从录屏取Score颜色、撤回旧聚合授权，并补齐Life 10组件层级、Retryable/Selectable/Annotated完整Transform祖先链、13槽Button/NoteLaneEffect精确Transform，以及Score/Auto/AddScore/Slide/particle/terminal四向消费门；不复制APK、R1 PNG或生产输出，production不得读取fixture。
