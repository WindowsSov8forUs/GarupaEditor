...
- `reverse-snapshots/ordinary-visible-rendering/` 包含已推送 Reverse `80a173a2` 的普通Note动画与通用单人HUD contract/profile/oracle/closure/manifest及4个去重PNG；RhythmGameUI与sgm字体复用既有Score快照身份，不重复保存。该快照只授权portable production消费，不声明Garupa已消费或设备GPU/framebuffer exact。
