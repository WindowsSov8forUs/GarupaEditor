...
- `reverse-snapshots/hud-complete/` 包含已推送Reverse `07bf4e7c`的完整选定HUD scene/component/resource/logic/primitive/runtime/closure七项JSON；它以current UITexture组件引用冻结`Flash→ss_overlay`、`BigStar_1/2→sss_star_long`，不复制level3、原作PNG或设备帧，production不得读取fixture。
