...
const outputRoot = sharedOutputRoot ?? mkdtempSync(join(tmpdir(), "garupa-original-live-settings-tests-"));
...
try {
  ...
  run(process.execPath, [join(outputRoot, "src", "simulator", "testing", "originalLiveSettings.test.js")]);
  run(process.execPath, [join(testingRoot, "verifyOriginalLiveSettingsStatic.mjs")]);
