...
if (!/export interface SimulatorChartDataPackage \{\s*readonly chart: GarupaChartJson;\s*readonly bgm: Uint8Array;\s*readonly isFullLength: boolean;\s*readonly laneCount: SimulatorProductLaneCount;\s*\}/m.test(publicContracts) ||
  ...
  throw new Error("Public chart is not exact Garupa-JSON/BGM-bytes/isFullLength/laneCount");
}
