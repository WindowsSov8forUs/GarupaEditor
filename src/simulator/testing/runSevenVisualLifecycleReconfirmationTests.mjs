...
const outputRoot = sharedOutputRoot ?? mkdtempSync(join(tmpdir(), "garupa-seven-visual-lifecycle-"));
...
try {
  ...
  run(process.execPath, [join(outputRoot, "src", "simulator", "testing", "sevenVisualLifecycleReconfirmation.test.js")]);
  run(process.execPath, [join(testingRoot, "verifySevenVisualLifecycleReconfirmationStatic.mjs")]);
} finally {
