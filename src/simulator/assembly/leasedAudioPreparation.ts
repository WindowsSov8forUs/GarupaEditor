...
const EXPECTED_CUES = new Set([
  ...
  "after_live", "SE_RESULT_SUCCESS", "SE_RESULT_FAILED", "SE_RESULT_SCORE_COUNT", "SE_RHYTHM_CLEAR", "SE_RHYTHM_FULLCOMBO", "SE_RHYTHM_GAYA", "SE_RHYTHM_TAP_SKILL", "bad", "miss",
...
export async function prepareLeasedAudioResources(
  ...
  const dynamic: (AudioFixedSeResourceProfile | AudioResultBgmResourceProfile)[] = [];
  ...
  for (const pack of packs) {
    ...
    for (const value of portableAudio) {
      ...
      const loop = value.cue === "after_live" ? Object.freeze({ ...resultMedia.bgm.loop })
        : value.cue === "SE_RESULT_SCORE_COUNT" ? Object.freeze({ start: 0, end: metadata.sampleFrames })
        : value.cue === "SE_RHYTHM_TAP_LONG"
      ...
      if (value.cue === "after_live" && pack.logicalResource !== "sound/result") return invalid("simulator.audio.leased-result-owner");
      ...
      const profile: AudioFixedSeResourceProfile | AudioResultBgmResourceProfile = value.cue === "after_live"
        ? Object.freeze({ ...metadataProfile, role: "result-bgm", logicalId: "sound/result", cue: "after_live",
            signal: "portable-equivalent-lossy" })
        : Object.freeze({ ...metadataProfile, role: "se", cue: value.cue,
