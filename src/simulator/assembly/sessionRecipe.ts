...
class RecipeOwnedSession implements SimulatorOwnedSession {
  ...
  private naturalCompletionReadyForResult = false;
  private resultPresentationActive = false;
  private finalSnapshot: SimulatorSnapshot | null = null;
  ...
  async step(
  ...
  ): Promise<SimulatorOwnedSessionStepResult> {
    ...
    if (this.resultPresentationActive) {
      const result = this.engine.advanceResult(deltaTimeSeconds, manualFrame);
      if (result.status !== "ok") return rejectedStep(result);
      if (result.value === "replay") {
        const replayed = await this.engine.replayResult();
        if (replayed.status !== "ok") return rejectedStep(replayed);
        this.resultPresentationActive = false;
        this.naturalCompletionPresentationActive = false;
        this.naturalCompletionReadyForResult = false;
        this.finalSnapshot = null;
        this.gameOverElapsedSeconds = null;
      }
      return result.value === "exit"
        ? Object.freeze({ status: "closed" as const, report: this.finish("completed", null) })
        : Object.freeze({ status: "running" as const });
    }
    if (this.engine.getNaturalCompletionClearStatus() !== null) {
      this.naturalCompletionPresentationActive = true;
      if (this.naturalCompletionReadyForResult) {
        const snapshot = this.engine.snapshot();
        if (snapshot.status !== "ok") return rejectedStep(snapshot);
        const scoreLife = snapshot.value.managers.scoreLifeState;
        const record = scoreLife?.record;
        const clearStatus = this.engine.getNaturalCompletionClearStatus();
        const control = this.engine.getTimelineControlState();
        if (control.status !== "ok") return rejectedStep(control);
        if (record === undefined || scoreLife === null || scoreLife === undefined || clearStatus === null) return rejectedStep(integrityFailure(
          "simulator.result.final-record-missing", "Natural completion requires the final committed score record."));
        const shown = await this.engine.showResult(Object.freeze({ record, isAutoLive: control.value.mode.isAutoLive,
          isEnablePractice: control.value.mode.isEnablePractice,
          isDemoPlayMode: control.value.mode.isDemoPlayMode,
          clearStatus, scoreThresholds: scoreLife.scoreGauge.thresholdProfile }));
        if (shown.status !== "ok") return rejectedStep(shown);
        this.finalSnapshot = snapshot.value;
        this.resultPresentationActive = true;
      ...
      // Publish the last outro frame before the result scene takes ownership.
      this.naturalCompletionReadyForResult = advanced.value;
      return Object.freeze({ status: "running" as const });
    }
  ...
  getControlState(): SimulatorAssemblyResult<SimulatorTimelineControlState> {
    ...
    return accepted(!this.naturalCompletionPresentationActive && this.gameOverElapsedSeconds === null
      ...
      : Object.freeze({ ...state.value, playable: false, terminalPresentationActive: true,
          resultPresentationActive: this.resultPresentationActive }));
  }
  ...
  close(
    ...
    return this.finish(reason === "user-closed" && this.resultPresentationActive ? "completed" : reason, failure ?? null);
  }
  ...
  private finish(
    ...
    const snapshot = this.finalSnapshot === null ? this.engine.snapshot() : ok(this.finalSnapshot);
    ...
    const report = Object.freeze({
      ...
      result: value === null || record === null ? null : Object.freeze({
        ...
        judgementCounts: Object.freeze([...record.resultCounts]) as readonly [number, number, number, number, number],
