...
export async function assembleSimulatorResources(
  ...
  const resultAudio = await prepareSourceAudioPackage("sound/result", lease);
  if (resultAudio.status === "rejected") return rejectedWithCleanup(resultAudio, rollback());
  const audio = await prepareLeasedAudioResources(
    chartAudio,
    Object.freeze([...skinPacks.value, commonAudio.value, commonSe.value, resultAudio.value]),
