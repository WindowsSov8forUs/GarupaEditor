import { groupMultipleDirectionalInformationList, isNonPlayableCommand } from "../chart/noteGraph";
import {
  FrontNoteType,
...
export function createConstructedChartScoringPlan(
  ...
  for (const batch of chart.noteBatches) {
    for (const source of batch.informationList) {
      ...
      if (!source.isInvisible) candidates.push(head);
      ...
      if (source.gameNoteType === GameNoteType.SlideA ||
        ...
        for (let index = 0; index < visible.length; index += 1) {
          ...
          candidates.push({
            source: child,
            aliases: [child],
            phase: child === source.slideNoteList[source.slideNoteList.length - 1] ? "tail" as const : "intermediate" as const,
  ...
  if (candidates.length > 0x7fffffff) {
    return integrityFailure(
      "score.plan.invalid-scoring-unit-count",
      "CS-V1 requires a non-negative Int32 count derived only from chart-owned scoring units.",
    );
  }
...
function isScoringRoot(source: NoteInformation): boolean {
  return !isNonPlayableCommand(source) && source.gameNoteType !== GameNoteType.None &&
