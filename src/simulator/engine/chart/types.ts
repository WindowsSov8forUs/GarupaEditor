...
export interface NoteInformation {
  ...
  /** Presentation-only Add member bound to one authored nonterminal Slide node. */
  readonly directionalSlideConnection?: Readonly<{ rootIndex: number; connectionIndex: number; memberOffset: number }>;
  /** Approved affiliation for expanded Directional member groups. */
  readonly directionalMemberRootIndex?: number;
  readonly directionalMemberOffset?: number;
  /** Approved Garupa semantics: retain hidden endpoints in the shared Slide owner. */
  readonly hiddenSlideEndpoints?: true;
  /** Approved nonterminal Slide gesture; one authored node, owned by NoteSlide. */
  readonly slideGesture?: Readonly<{ direction: "Left" | "Right" | null; width: number; noteType: 3 | 9 | 10 }>;
  /** Signed lane distance from this node's incoming point to its outgoing point. */
  ...
  /** Original BMS supplies Int32; Garupa may retain fractional hundredth-lane distances. */
...
export interface ChartConstructionResult {
  ...
  /** Original #HABAHIRO chart mode, not a summary of individual note widths. */
