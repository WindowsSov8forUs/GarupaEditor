...
export function calculateNativeParticleRuntimeTransform(
  ...
  if (scalingMode === 0) {
    ...
    basis = scaledColumns({ rotation, scale: scalingMode === 2 ? [1, 1, 1] : self.scale });
  }
  ...
  return {
    ...
    scalingModeScale: scalingMode === 0
      ? calculateNativeParticleHierarchyScale(self, rootToImmediateParents) : scalingMode === 2 ? [1, 1, 1] : self.scale,
  };
}
...
export function calculateNativeParticleLocalBillboardBasis(
  ...
  if (scalingMode === 0) {
    ...
    scale = scalingMode === 2 ? [1, 1, 1] : self.scale;
  }
