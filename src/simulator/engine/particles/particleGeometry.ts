...
export function buildCurrentParticlePrimitives(
  ...
  for (const [key, rows] of grouped) {
    const bounds = nativeRendererBounds(systemSamples.get(key)!, scene);
...
function buildPrimitive(
  ...
  const outerScale: Vector3 = [1, 1, 1];
  ...
  const source = isStretched
    ? stretchedBillboard(binding, sample, worldCenter, outerScale, scene)
...
function stretchedBillboard(
  ...
  const arithmetic = calculateNativeStretchArithmetic({
    cameraPosition,
    cameraVelocity: stretchCameraVelocity(sample),
    sizeY: rawSize[1],
...
function stretchCameraVelocity(sample: ParticleRenderSample): Vector3 {
  // Simulation publishes world velocity; only the camera-space Z reflection remains.
  const velocity = bitsVector3(sample.velocity);
...
function particleWorldCenter(sample: ParticleRenderSample, transform: ParticleOwnerTransform): Vector3 {
  ...
  return bitsVector3(sample.position);
}
...
function nativeRendererBounds(sample: ParticleRenderSample, scene: ParticlePixiSceneProfile): ParticleNativePrimitiveBounds {
