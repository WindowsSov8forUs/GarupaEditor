  ...
  constructor(
    ...
    for (const bundle of profile.bundles) {
      for (let bundleOrdinal = 0; bundleOrdinal < bundle.systems.length; bundleOrdinal += 1) {
        ...
        if (definition.sourceOrdinal !== bundleOrdinal ||
          bundle.key !== "game-clear" && (!Number.isSafeInteger(definition.nativePlayOrdinal) || definition.nativePlayOrdinal! < 0 ||
            typeof definition.nativePlayActive !== "boolean")) {
          throw fault("particle.simulation.source-ordinal-drift", "Native-semantic Schema 2 requires contiguous component order and source-bound gameplay Play traversal and hierarchy activity.");
        }
  ...
  playRoot(
    ...
    if (owner !== undefined) {
      if (owner.root !== root || !sameParticleInstance(owner.instance, instance) ||
        !sameParticleSetupScale(owner.particleSystemSetupScale, instanceParticleSystemSetupScale(instance))) {
  ...
  playRootSystems(
    ...
    if (owner === undefined) {
      ...
      owner = {
        ...
        particleSystemSetupScale: instanceParticleSystemSetupScale(instance),
    ...
    const selected = [...new Set(selectedSystemIds)].map((identity) => {
      ...
      return record;
    }).filter((record) => instance.kind === "game-clear" ||
      record.definition.nativePlayActive === true)
      .sort((left, right) => instance.kind === "game-clear"
...
function particleOwnerParents(instance: ParticleInstanceIdentity): readonly ParticleHierarchyPositionTransform[] {
  ...
  if (instance.kind === "game-clear" || instance.kind === "result-ui") {
    const owner = instance.ownerTransform;
    if (owner === undefined || owner.source !== (instance.kind === "result-ui" ? "result-ui-root" : "game-clear-ui-root") ||
...
function sameParticleInstance(left: ParticleInstanceIdentity, right: ParticleInstanceIdentity): boolean {
  ...
  if (left.kind === "game-play-button" && right.kind === "game-play-button") {
    return left.isHalfButton === right.isHalfButton &&
...
function instanceParticleSystemSetupScale(
  ...
  const value = particleFloat32FromBits(instance.particleSystemSetupScaleBits);
