...
export class AudioCommandProducer {
  ...
  preflightResultBgm(): SimulatorResult<AudioOwnerTransaction> {
    return this.preflightCommands([{ kind: "bgm.start-result", cue: "after_live", fade_seconds: 0.25 }]);
  }

  preflightResultEvaluation(failed: boolean): SimulatorResult<AudioOwnerTransaction> {
    return this.preflightCommands([oneShot(failed ? "SE_RESULT_FAILED" : "SE_RESULT_SUCCESS", "one-shot-0")]);
  }

  preflightResultCounting(active: boolean): SimulatorResult<AudioOwnerTransaction> {
    return this.preflightCommands([active
      ? { kind: "se.start-owned-loop", cue: "SE_RESULT_SCORE_COUNT", owner_key: "result-count",
          volume_bits: "0x3F800000", fade_in_bits: "0x00000000" }
      : { kind: "se.fade-owned-loop", owner_key: "result-count", target_bits: "0x00000000",
          duration_bits: "0x00000000", stop_at_zero: true }]);
  }
...
function judgementCue(
  ...
  if (((1 << entry.noteType) & flickMask) !== 0) {
    ...
    if (entry.noteType === 7 || entry.noteType === 10) {
      ...
      // playFlickJudgeSE's zero-count logging branch rejoins the single cue.
      if (count === 0 || count === 1) return ok("directional_fl");
      if (count === 2) return ok("directional_fl_2");
      if (count >= 3) return ok("directional_fl_3");
      return rejected(
        "audio.judgement.invalid-multiple-directional-count",
        "Multiple Directional cue selection requires a positive owned group count.",
      );
    }
    return ok("flick");
  }
