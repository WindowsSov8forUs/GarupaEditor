...
export function noteSlideFlashBinding(resources: RenderEngineResourceBindings, width: number, center: number, habahiro: boolean) {
  return noteLongFlashBinding(resources, slideCenteredLaneSuffix(width, center, habahiro), habahiro);
}

/** ExecTouchBegan replaces the head body with the same centered lane selection as its Flash. */
export function noteSlideHeldBodyBinding(resources: RenderEngineResourceBindings, width: number, center: number, habahiro: boolean) {
  return noteBodyBinding(resources, "note_long", slideCenteredLaneSuffix(width, center, habahiro), width, habahiro)!;
}

export function slideCenteredLaneSuffix(width: number, center: number, habahiro: boolean): string {
  // Outside the original atlas domain, retain the original single-lane selection.
  const size = habahiro && Number.isInteger(width) && width >= 1 && width <= 7 ? width : 1;
  const start = Math.floor((7 - size) / 2) + (size % 2 === 0 && center > 3 ? 1 : 0);
  return Array.from({ length: size }, (_, index) => start + index).join("_");
}
