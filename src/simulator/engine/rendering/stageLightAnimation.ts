export interface StageLightFadeInput {
  ...
  readonly colorTable?: readonly (readonly number[])[];
}
...
export class StageLightAnimation {
  ...
  advance(deltaSeconds: number): void {
    ...
    if (this.elapsed >= this.input.duration) {
      ...
      this.index = (this.index + 1) % (this.input.colorTable === undefined ? TABLE_LENGTH : this.input.colorTable.length);
  ...
  private target(): readonly number[] {
    return this.input.colorTable === undefined ? (this.index === this.skyIndex ? SKY_BLUE : BLUE) : this.input.colorTable[this.index]!;
  }
}
