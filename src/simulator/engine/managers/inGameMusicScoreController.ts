...
export class InGameMusicScoreController {
  ...
  constructor(chart: ChartConstructionResult, noteArrivalSeconds: number) {
    ...
    for (const node of profile?.nodes ?? []) {
      if (axisGroups.has("#Global") || axisGroups.has(node.timingGroup)) {
        for (const source of node.runtimeMembers ?? []) axisSources.add(source);
      }
    }
    for (const batch of chart.noteBatches) {
      if (batch.informationList.length > 0 && batch.informationList.every(source => axisSources.has(source))) this.axisBatches.add(batch);
    }
    ...
    const launcherLead = noteArrivalSeconds * this.basicBpmValue / 240 * MUSIC_BAR_DIVISION_COUNT;
  ...
  validateAdvanceSequence(
    ...
    if (
      ...
      return integrityFailure(
        ...
        "The portable scheduler must reject an advance sequence that cannot retain finite music positions.",
      );
    }
    ...
    for (let index = 0; index < substepCount; index += 1) {
      ...
      if (
        ...
        return integrityFailure(
          "music-score.non-finite-advance",
          "The portable scheduler must reject an advance sequence before it writes a non-finite music position.",
        );
      }
  ...
  canActivateBatch(batch: NoteBatchInformation): SimulatorResult<boolean> {
    ...
    const first = batch.informationList.find(
      (note) => note.buttonType !== ButtonType.None || note.laneSpan !== undefined || isBpmCommand(note),
    );
    if (first === undefined) {
      return ok(true);
    }
...
export function advancePosition(
  ...
  const secondsPerPosition = 240 / bpm / MUSIC_BAR_DIVISION_COUNT;
  let nextProgress = beatProgress + deltaTimeSeconds / secondsPerPosition;
  let nextBar = bar | 0;
  if (nextProgress >= MUSIC_BAR_DIVISION_COUNT) {
    nextProgress -= MUSIC_BAR_DIVISION_COUNT;
    nextBar = (nextBar + 1) | 0;
  }
...
function absolutePosition(position: MusicPosition): number {
  return position.beatProgress + MUSIC_BAR_DIVISION_COUNT * position.bar;
}
