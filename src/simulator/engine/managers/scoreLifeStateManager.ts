  ...
  static create(
    ...
    if (!validScoringPlan(scoringPlan)) {
      return integrityFailure(
        ...
        "Score/Life initialization requires one CS-V1 plan with a non-negative chart-owned unit count and exact scoreMaximum.",
      );
    }
...
function validScoringPlan(plan: SimulatorScoringPlan): boolean {
  return plan !== null && typeof plan === "object" &&
    plan.ruleSetId === NORMALIZED_SCORE_RULESET_ID && Array.isArray(plan.units) &&
