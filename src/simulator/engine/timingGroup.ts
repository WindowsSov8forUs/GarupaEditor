...
export function axisAtMs(group: TimingGroupDef | null | undefined, elapsedMs: number): number {
  ...
  for (const change of group.changes) {
    if (elapsedMs + EPSILON < change.atMs) {
      break;
    }
    speed = change.speed;
    pos = change.pos;
...
export function buildTimingGroupDefs(
  ...
  internalToRuntimeGroup: Map<string, number>;
} {
  ...
  for (const event of sourceEvents) {
    ...
    if (!Number.isFinite(atMs) || !Number.isFinite(speed)) {
      continue;
    }
  ...
  for (const [timingGroup, list] of grouped.entries()) {
    list.sort((left, right) => {
      if (Math.abs(left.atMs - right.atMs) > EPSILON) {
  ...
  for (let index = 0; index < usedGroups.length; index += 1) {
    ...
    const events = grouped.get(internalGroup) ?? [];
...
export function findVisibilityWindows(
  ...
  const boundaries = [
    startLimit,
    ...changes.map((change) => change.atMs).filter((atMs) => atMs > startLimit && atMs < endLimit),
    endLimit,
  ].sort((a, b) => a - b);
  ...
  for (let index = 0; index < boundaries.length - 1; index += 1) {
    ...
    if (Math.abs(delta) <= EPSILON) {
      if (startAxis >= lower - EPSILON && startAxis <= upper + EPSILON) {
        pushWindow(windows, startMs, endMs);
      }
      continue;
    }
    ...
    if (hi + EPSILON < 0 || lo - EPSILON > 1 || hi - lo <= EPSILON) {
      continue;
    }
