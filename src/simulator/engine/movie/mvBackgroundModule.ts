...
export class MvBackgroundModule {
  ...
  stepBeforeSound(deltaTimeSeconds: number): SimulatorResult<boolean> {
    if (!this.available() || !this.beforeStarted ||
      ...
      return rejected("movie.background.invalid-before-sound-step", "The pre-sound MV wait consumes finite non-negative engine delta after MovieBeforeSound starts.");
    }
  ...
  step(deltaTimeSeconds: number): SimulatorResult<void> {
    if (!this.available() || !Number.isFinite(deltaTimeSeconds) || deltaTimeSeconds < 0) {
      return rejected("movie.background.invalid-step", "MV background consumes only initialized finite non-negative engine delta.");
    }
    const cover = this.manager.advanceDarkCover(deltaTimeSeconds);
    if (cover.status !== "ok") return cover;
