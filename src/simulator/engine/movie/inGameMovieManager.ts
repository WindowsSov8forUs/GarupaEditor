...
export class InGameMovieManager {
  ...
  stepNegativeDelay(deltaTimeSeconds: number, targetSeconds: number): SimulatorResult<boolean> {
    if (!this.available() || !isFiniteNonNegative(deltaTimeSeconds) ||
      !isFiniteNonNegative(targetSeconds)) {
      return rejected("movie.manager.invalid-delay-step", "Negative MV delay consumes only finite non-negative engine delta and target values.");
    }
  ...
  advanceDarkCover(deltaTimeSeconds: number): SimulatorResult<void> {
    if (!this.available() || !isFiniteNonNegative(deltaTimeSeconds)) {
      return rejected("movie.manager.invalid-dark-cover-step", "MV dark-cover tween consumes only initialized finite non-negative engine delta.");
    }
