...
export interface GarupaProductNode {
  ...
  /** The NoteManager root, populated when chart construction binds this input. */
...
export function projectedNodeLane(node: GarupaProductNode): number {
  ...
  if (node.type === "Hidden" && source?.virtualLaneDirection) {
    return node.lane + Math.floor((node.width - 1) / 2);
  }
