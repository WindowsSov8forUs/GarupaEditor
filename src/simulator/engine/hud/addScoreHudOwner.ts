...
export class AddScoreHudOwner {
  ...
  createState(value: number, playOffset: number): RenderAddScoreHudState {
    return Object.freeze({
      value,
      alpha: Math.fround(0.6),
      localXOffset: 0,
      poolIndex: (this.poolIndex + playOffset) % 4 as 0 | 1 | 2 | 3,
      depth: (this.depth + playOffset) % 8 as 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7,
    });
  }

  commit(playCount: number): void {
    this.poolIndex = (this.poolIndex + playCount) % 4;
    this.depth = (this.depth + playCount) % 8;
  }
}
