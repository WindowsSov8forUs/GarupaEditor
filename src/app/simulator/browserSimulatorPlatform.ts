...
export async function createBrowserSimulatorPlatform(input: {
  ...
  readonly beginResultPreparation: () => SimulatorResultPreparation;
  ...
  const platform: AutonomousSimulatorPlatformCapabilities = Object.freeze({
    ...
    beginResultPreparation() {
      pointerInput.clearPresentationPointers();
      const loading = input.beginResultPreparation();
      let finished = false;
      return {
        ready: loading.ready,
        report: loading.report,
        finish(present: boolean) {
          if (finished) return;
          finished = true;
          try {
            if (!disposed && present) {
              // Upload/draw the ready destination while Loading still covers
              // the canvas; its first visible frame must not be stale gameplay.
              graphics.render();
              scheduler.resetClock();
            }
          } finally { loading.finish(present); }
        },
      };
    },
  });
...
class BrowserPointerInputSource implements SimulatorRuntimeInputSource {
  ...
  clearPresentationPointers(): void {
  ...
  consume(_sequence: number, controlState: SimulatorTimelineControlState, surface: SimulatorSurfaceState): SimulatorAssemblyResult<SimulatorRuntimeInputBatch> {
    ...
    for (const [pointerId, pointer] of this.pointers) {
      ...
      if ((phase === ManualTouchPhase.Ended || phase === ManualTouchPhase.Canceled) && pointer.pending.length === 0) {
  ...
  private readonly onPointerMove = (event: PointerEvent) => {
    ...
    const movement = { phase: ManualTouchPhase.Moved, position: pointer.position };
    const last = pointer.pending.length - 1;
    // The engine samples one position per finger and outer frame. Keep the
    // latest movement without queuing browser event frequency as gameplay time.
    if (pointer.pending[last]?.phase === ManualTouchPhase.Moved) pointer.pending[last] = movement;
    else pointer.pending.push(movement);
  };
  private readonly onPointerUp = (event: PointerEvent) => this.finishPointer(event, ManualTouchPhase.Ended);
  private readonly onPointerCancel = (event: PointerEvent) => this.finishPointer(event, ManualTouchPhase.Canceled);
  private finishPointer(event: PointerEvent, phase: typeof ManualTouchPhase.Ended | typeof ManualTouchPhase.Canceled): void {
    const pointer = this.pointers.get(event.pointerId);
    if (pointer === undefined || pointer.terminal) return;
    pointer.position = this.position(event);
    pointer.terminal = true;
    pointer.pending.push({ phase, position: pointer.position });
  }
  ...
  private readonly onWindowBlur = () => {
    for (const pointer of this.pointers.values()) {
      ...
      pointer.terminal = true;
      pointer.pending.push({ phase: ManualTouchPhase.Canceled, position: pointer.position });
    }
  };
