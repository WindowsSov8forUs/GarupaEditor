...
import { createSerializedButtonPressCover, createSerializedButtonSprite } from "./SerializedButton";
...
export interface DialogLabelLayout { readonly spacingX: number; readonly spacingY: number; readonly maxLines: number; }
interface LabelProfile extends SpriteProfile, DialogLabelLayout { readonly fontSize: number; readonly pivot: "left" | "center"; }
export interface SerializedDialogProfile {
  ...
  readonly buttons: readonly {
    ...
    readonly spacingX: number; readonly spacingY: number; readonly maxLines: number;
  }[];
}
...
export type DialogTextFactory = (value: string, size: number, fill: DialogColor, label: string,
  width: number, height: number, pivot: "left" | "center", layout: DialogLabelLayout) => Text;
...
export function createSerializedDialog(
  ...
  borders: Readonly<{ window: Border; header: Border; button: Readonly<{ pink: Border; gray: Border }> }>,
  ...
  createText: DialogTextFactory,
    ...
    const title = createText(titleText, profile.title.fontSize, palette.title,
      ...
      profile.title.size[0], profile.title.size[1], profile.title.pivot, profile.title);
    ...
    const content = createText(contentText, profile.content.fontSize, palette.content,
      ...
      profile.content.size[0], profile.content.size[1], profile.content.pivot, profile.content);
    ...
    if (profile.annotation !== undefined && annotationText !== null && paths.annotation !== undefined) {
      ...
      const annotation = createText(
        ...
        profile.annotation.size[0], profile.annotation.size[1], profile.annotation.pivot, profile.annotation,
      );
    ...
    profile.buttons.forEach((buttonProfile, index) => {
      ...
      const style = buttonProfile.spriteName === "button_pink" ? "pink" : "gray";
      const legacy = `${profile.identity}:${buttonProfile.identity}`;
      const button = createSerializedButtonSprite(textures[style], buttonProfile.spriteSize,
        borders.button[style], { x: 0.5, y: 0.5 }, legacy);
      ...
      const caption = createText(
        ...
        buttonProfile.labelSize[0], buttonProfile.labelSize[1], "center", buttonProfile,
      );
      caption.position.set(buttonProfile.labelPosition[0], buttonProfile.labelPosition[1]);
      caption.zIndex = buttonProfile.labelDepth;
      const pressedCover = createSerializedButtonPressCover(button, `${legacy}-pressed`);
...
export class SerializedDialogTransition {
  private readonly motion = new SerializedDialogMotion();
  ...
  setOpen(open: boolean, now: number): void {
    this.motion.scale = this.window.scale.x;
    this.motion.alpha = this.cover.alpha;
    this.motion.setOpen(open, now);
  }

  update(now: number): boolean {
    const visible = this.motion.update(now);
    this.window.scale.set(this.motion.scale);
    this.cover.alpha = this.motion.alpha;
    return visible;
  }
}
