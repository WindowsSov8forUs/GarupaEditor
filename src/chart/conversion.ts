...
export function parseGarupaChartJson(input: unknown): GarupaChartJson {
  ...
  input.forEach((rawItem, itemIndex) => {
    ...
    if (rawType === "Slide") {
      ...
      if (rawConnections.length < 2) {
        throw new Error(`${label}.connections must contain at least two nodes`);
      }
...
function convertBestdoriItemToGarupa(item: BestdoriV2ChartJsonItem): GarupaChartJsonItem | null {
  if (item.type === "BPM") {
    return {
      type: "BPM",
      beat: item.beat,
  ...
  const connections = item.connections.map((connection) => convertBestdoriSlideConnectionToGarupa(connection));
  if (connections.length === 1) {
    const only = connections[0]!;
    return only.type === "Hidden" ? null : { ...only, type: only.type };
  }
  return { type: "Slide", connections };
}
...
export function convertBestdoriV2ToGarupaChartJson(
  ...
  const converted = parsed.map((item) => convertBestdoriItemToGarupa(item))
    .filter((item): item is GarupaChartJsonItem => item !== null);
