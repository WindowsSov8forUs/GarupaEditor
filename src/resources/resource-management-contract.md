...
The project-wide runtime policy is [`../runtime-contract-policy.md`](../runtime-contract-policy.md). Evidence gaps never qualify as resource failures. The manager validates non-empty bytes, complete package manifests, unique safe relative paths, byte length, SHA-256 and atomic transaction publication. Persistent content is reverified when acquired. Corrupted or incomplete resources are rejected and quarantined; there is no alias, silence, white texture, stale partial file or source-class fallback. A missing or incompatible required resource makes only the requested operation unavailable: the prior selection, current project and editor remain active, and no half-prepared consumer receives ownership.

Every physical Builtin is an explicit non-inlined Vite URL asset. The generated source manifest owns the complete `src/assets` set; the application catalog owns exactly the source-manifest difference after removing the Simulator provenance set, and the Simulator catalog owns exactly that provenance set. The catalog sets are disjoint and their union is the source manifest. These relationships, rather than hand-maintained counts, are verified on every resource test. The bundler may emit content-hashed filenames, but returned bytes must retain the manifest length and SHA-256. Production build acceptance hashes `dist/assets` after Vite completes; successful compilation without that post-build byte gate is not a releasable resource payload. A runtime mismatch remains a typed integrity failure for the affected action and reports the logical path plus expected/observed integrity; the manifest must never be regenerated from transformed output merely to silence the failure.
...
A source-bound semantic consumer may additionally require the lease revision and exact file receipt tuples. It must rehash each read before using a committed semantic sidecar and must prove that every sidecar component/texture relation belongs to that revision. Expected integrity always comes from the Snapshot/Lease record or an independently pinned Builtin manifest; computing a digest from the bytes under test proves only self-consistency. One immutable preparation may be shared by multiple backends, but changing the provider or preflight capability after that preparation is an ownership violation.

Simulator package views share successful reads only within the same live lease and revision; rejected reads are not cached and byte consumers still receive owned copies. Audio and particle preparation use asynchronous host SHA-256 while retaining the independent expected digest checks. AudioBuffer reuse remains keyed by verified content; Retry rebuilds mutable gameplay/backend owners. Tauri Base64 decoding writes directly into one allocated byte array instead of allocating through a per-character iterator.
...
The authorized Simulator migration is governed by [`simulator-resource-integration-contract.md`](./simulator-resource-integration-contract.md). Simulator continues to consume source-neutral Snapshot/Lease bytes; current-session ownership does not change Public Schema 13, original behavior, resource resolver or compatibility validation.

## Simulator loading-window handoff

Desktop launch allocates a request identity and opens the loading window before preparing the media snapshot. Window readiness and descriptor readiness may arrive in either order; the descriptor is published once both are ready. Closing or failing that window abandons the handoff, releases any published lease, and releases a lease that finishes preparation afterward. Mobile navigation continues to wait for its complete route payload. Default Stage lookup uses the prepared/cached catalog; a cached lookup miss retains the existing refresh-on-miss behavior.

The fixed loading screen uses registered build URLs and is shown after its images/fonts and comic frame canvas are ready, before initializing the persistent resource backend in that WebView. One loading owner retains the same screen across resource bootstrap, lazy module loading and Simulator assembly; those handoffs must not remount the canvas or restart its animations. Mobile creates that owner per Simulator route. Desktop Simulator routes do not import the editor controller; shared application styles remain available during loading.

The gauge counts eight equally weighted preparation stages: application resources, module, launch media/input and audio-context readiness, graphics platform, resource availability, acquired resource lease, rendering preparation, and the first presented cover frame. Availability reports unique ready resources; rendering reports the completed source-package, common-render, skin-render and renderer preparation steps. Only successful work advances the gauge; cached stages may finish immediately. This is completed-work progress, not downloaded-byte progress or a remaining-time estimate. It cannot reach completion from availability alone. The first actual cover frame synchronously removes the loading screen without waiting for the launch owner to become running; remaining audio/particle preparation continues during the original cover presentation as specified in the rendering contract. No timer smoothing, artificial 100-percent hold or packaged asset changes are involved.
