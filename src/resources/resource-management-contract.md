...
The fixed loading screen uses registered build URLs and is shown after its images/fonts are ready, before initializing the persistent resource backend in that WebView. Simulator initialization still waits for the backend and all required Snapshot/Lease checks. Desktop Simulator routes do not import the editor controller; the editor loads on demand, with shared application styles available to the loading screen. This changes preparation order, not progress semantics or packaged asset bytes.
