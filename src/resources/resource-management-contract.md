...
All 43 physical Builtins are explicit non-inlined Vite URL assets. The source manifest remains authoritative across development and production: the bundler may emit a content-hashed filename, but the bytes returned by that URL must retain the manifest length and SHA-256. Production build acceptance hashes `dist/assets` after Vite completes; successful compilation without that post-build byte gate is not a releasable resource payload. A runtime mismatch remains a typed integrity failure for the affected action and reports the logical path plus expected/observed integrity; the manifest must never be regenerated from transformed output merely to silence the failure.
