...
`npm run build` is not complete after Vite compilation alone: `resources:verify-production-assets` hashes `dist/assets` and requires all 42 logical Builtins to retain their source-manifest byte length and SHA-256. This closes source-to-production transformations such as SVG Data URL normalization while retaining the typed runtime byte-integrity check; a mismatch blocks only the affected resource action.
