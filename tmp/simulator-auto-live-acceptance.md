...
- invisible 与 visible current child 均先独立读取 adjusted position、检查有限值并执行 `adjusted < child.absolutePos` 返回；root未crossing、root equal但child未到及child前一Float32均保持cursor。只有child equal/after，invisible才不产生OneFrame并推进一个。
...
| AL10 | 通过 | intermediate源序成立；invisible从真实Move覆盖root/child前一Float32与equal，到点前cursor/judged/slot不变，到点后单次skip且不占槽 |
