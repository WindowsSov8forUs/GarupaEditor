...
final R6与HAB degraded消费后的 production-consumption 矩阵为：**19 closed/closed-current-subset/closed-degraded、18 partial、3 blocked**。runner 全绿只说明当前已实现子集及失败边界一致，不等于 PR01–PR40 全部正向实现。

## 2. 锁定状态

- Garupa 分支：`codex/refactor-simulator-implementation`
- 验收运行 HEAD：`1a9a626`（R6 freeze `9092c27` + production `cc77b31`/`35c4535` + oracle `f3e300f`/`1a9a626`）；文档HEAD `cf88f9c`
- Reverse 分支：`main`
- Garupa消费的Reverse证据基线：`64a88a8821d20eb1cc43003a9bd959c892d40e12`
- Reverse当前远端HEAD：`64a88a88`（final R6五条clean trace与保守profile已晋升）
- Garupa 与远端差异：`0 0`
- Reverse 与远端差异：`0 0`
...
- entries：826；methods/layouts/enums：673/32/19；
...
- final R6：190,401 events、2,492 aggregate frames、26 observed owners、1个new authorized gate；
...
| RP05 | 部分 | ordinary root与显式field/judge/mask producer存在；HAB degraded lane-change已接，field host scene仍未接。 |
...
| RP08 | 部分 | Score/Combo/Life、Result lifetime及ScoreUp 1–4完成；AddScore round-robin与Crescendo text未完成。 |
| RP09 | 部分 | Combo/Life Heal portable clock、generic/Score Skill完成；Guard/NeverDie/Judge链未完成。 |
...
| RP14 | **不通过** | 3个blocked production case仍缺正向实现。 |

## 6. PR01–PR40 production矩阵

- **Closed / current subset / degraded（19）**：PR01–PR05、PR07、PR10、PR12–PR13、PR16–PR17、PR19、PR23、PR33、PR35–PR38、PR40。
- **Partial（18）**：PR06、PR08–PR09、PR11、PR15、PR18、PR20–PR22、PR24–PR29、PR31–PR32、PR34。
- **Blocked（3）**：PR14、PR30、PR39。
...
- PR19/PR40以显式degraded轨关闭：production chart在1728依次发出flash-start/change-lane并显示label；不关闭natural HAB runtime、current UnityFS或original frame。

## 7. 验证结果

从clean、已推送`1a9a626`串行执行：
...
- PR production verifier：`closed=19 partial=18 blocked=3 RP14=blocked`；
- render contracts：9组通过；
- actual Pixi、failure、resource、geometry、Long、HUD/mask/animation、ScoreUp、degraded label/lane-change：通过；
- HAB degraded production chart：重放至1728，4,902 commands、phase顺序与release通过；ordinary exact audit在Slide terminal按证据失败关闭；
...
1. PR14：advanced/threshold/material shader portable mapping；
2. PR30：Damage Guard/NeverDie warning suppression正向owner轨迹；
3. PR39：ordinary Slide Flick/Directional/Multiple terminal及完整`poppin_shuffle_special` exact重放。

此外18个partial case仍只代表current authorized subset；exact HAB UnityFS/natural R1/original frame继续open-not-claimed。
...
R4关闭PR07/PR12/PR17 current subset；R5/R6接入Result、Skill/ScoreGauge及ScoreUp 1–4，并保持Crescendo text/Guard/NeverDie等false。HAB degraded production replay关闭PR19/PR40 degraded轨；ordinary exact不借用proxy，PR39继续blocked。
