...
- `BS01–BS36` partial oracle已冻结：BS05/BS06/BS11为静态confirmed，19个case为partial，14个case blocked；`unknown_fields=155`、`blocking_findings=92`；D19/D24仍未关闭；
- production chart count独立oracle按10.1.4 `NoteManager.analyzeBMS`规则固定ordinary `979`与HABAHIRO `731`，部分推进D03/D19/BS01/BS02与D23，但尚未改写fixed-event case状态；
- `R1=4`且`business_state_gate=open`，不得实施B03–B12 production。

B02仍必须补齐D18/D20剩余范围、D19的155个unknown fields与92个blocking findings、D21、D22的score-decrease/Continue/seek/ReturnTime、D23剩余deck/start-data/master provenance及D24；不得把单Skill或Retry轨迹外推为Fever、guard、Never Die、多/重叠Skill或完整生命周期oracle。
...
`verify.mjs` 校验十三个Reverse commit、source/copy/index三方字节、manifest、冻结SHA256SUMS、静态contract/findings/closure、BMS/cache record、production chart count `979/731`、四条R1压缩raw trace、计划来源/动作、Skill生命周期/同帧冻结/once-heal、Retry-only安全边界与reset轨迹、BS01–BS36 partial oracle、观察型capture约束和业务门状态。
