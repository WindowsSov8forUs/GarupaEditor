...
- `BS01–BS36` partial oracle已冻结：BS05/BS06/BS11为静态confirmed，20个case为partial，13个case blocked；`unknown_fields=151`、`blocking_findings=91`；D19/D24仍未关闭；
...
B02仍必须补齐D18/D20剩余范围、D19的151个unknown fields与91个blocking findings、D21、D22的score-decrease/Continue/seek/ReturnTime、D23剩余deck/start-data/master provenance及D24；不得把单Skill或Retry轨迹外推为Fever、guard、Never Die、多/重叠Skill或完整生命周期oracle。
...
`verify.mjs` 校验十三个Reverse证据节点、source/copy/index三方字节、manifest、冻结SHA256SUMS、静态contract/findings/closure、BMS/cache record、production chart count `979/731`、四条R1压缩raw trace、计划来源/动作、Skill生命周期/同帧冻结/once-heal、Retry-only安全边界与reset轨迹、BS01–BS36 partial oracle、观察型capture约束和业务门状态。
