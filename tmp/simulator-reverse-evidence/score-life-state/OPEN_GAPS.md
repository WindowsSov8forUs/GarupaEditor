...
- `D19-partial`：BS01–BS36 partial oracle已生成并独立校验；BS01/BS05/BS06/BS11共4个confirmed，20个case partial，12个case blocked；`unknown_fields=135`、`blocking_findings=85`，因此D19仍required-before-code。
...
- `D10/D12/D14/D18-ordinary-Auto-partial`：5501事件零tail R1锁定979个one-note、strict/equal max retention、6个顺序Skill lifecycle/5个匿名alias及Life `1000→1200→1500` overheal；不外推为重叠Skill或Skill effect master rows。

## required-before-code

- `D18`剩余：Fever、guard、Never Die、重叠Skill及special-mode对象identity与before/after字段；ordinary顺序六Skill已锁定。
- `D19`剩余：BS01–BS36尚有135个unknown fields与85个blocking findings，必须补齐master/R1/failure证据后才能全部`unknown_fields=[]`。
...
fixed_event_unknown_fields=135
fixed_event_blocking_findings=85
