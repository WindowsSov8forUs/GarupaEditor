#!/usr/bin/env python3
"""Observe rehearsal pause/resume, Life/Game Over, and ReturnTime boundaries."""
...
TARGETS = {
    ...
    "CommandNoteManager.ReturnTime": 0x367CC44,
    "InGameMoveTimeController.returnTime": 0x3300E08,
    "NoteManager.ReturnTime": 0x37756A8,
}
...
function recordState(p){{return !p||p.isNull()?null:{{is_multi_game_over:u8(p,0x11),is_single_game_over:u8(p,0x12),score:u32(p,0x14),reserve_score:u32(p,0x1C),life:i32(p,0x20),max_life:i32(p,0x24),max_note_count:i32(p,0x2C),combo:i32(p,0x34),one_note:oneNote(p64(p,0x78)),event_one_note:oneNote(p64(p,0x80)),judge_counts:[i32(p,0x44),i32(p,0x48),i32(p,0x4C),i32(p,0x50),i32(p,0x54)]}};}}
...
hook('InGameRecord.AddIPower',{{onEnter(args){{this.self=args[0];this.add=args[1].toInt32();emit('InGameRecord.AddIPower.enter',{{add:this.add,record:recordState(this.self)}});}},onLeave(){{emit('InGameRecord.AddIPower.leave',{{add:this.add,record:recordState(this.self)}});}}}});
hook('InGameRecord.updateGameOverState',{{onEnter(args){{this.self=args[0];emit('InGameRecord.updateGameOverState.enter',{{record:recordState(this.self)}});}},onLeave(){{emit('InGameRecord.updateGameOverState.leave',{{record:recordState(this.self)}});}}}});
...
for(const name of ['CommandNoteManager.ReturnTime','NoteManager.ReturnTime']){{hook(name,{{onEnter(args){{emit(name+'.enter',{{snapshot_present:!args[1].isNull()}});}},onLeave(){{emit(name+'.leave');}}}});}}
hook('InGameMoveTimeController.returnTime',{{onEnter(args){{this.back=args[1].toUInt32();emit('InGameMoveTimeController.returnTime.enter',{{back_second:this.back}});}},onLeave(){{emit('InGameMoveTimeController.returnTime.leave',{{back_second:this.back}});}}}});
...
def main() -> int:
    ...
    completion_counts = {"one_note_leave": 0, "skill_finished_leave": 0}
    ...
    def drain_loop() -> None:
        ...
        while not stop.wait(0.2):
            try:
                ...
                for event in batch:
                    if event["kind"] == "InGameRecord.CalcOneNotesMaxScoreInfo.leave":
                        completion_counts["one_note_leave"] += 1
                    if event["kind"] == "SituationSkillManager.processOfSkillFinished.leave":
                        completion_counts["skill_finished_leave"] += 1
                if completion_action and completion_counts["one_note_leave"] >= completion_action["required_one_note_leave_count"] and completion_counts["skill_finished_leave"] >= completion_action["required_skill_finished_leave_count"]:
    ...
    output = {
        ...
        "capability": {"level":"R1","return_replacement":False,"memory_writes":False,"apk_modification":False,"managed_invocation":False,"input_injection":"one predeclared Android adb tap followed by passive wait-until-complete only","transport":{"kind":"explicit-remote","address":args.device_address}},
