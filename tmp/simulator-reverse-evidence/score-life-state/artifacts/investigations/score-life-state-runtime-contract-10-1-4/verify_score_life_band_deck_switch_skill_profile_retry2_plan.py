#!/usr/bin/env python3
"""Verify the no-Live-start anonymous band deck Skill profile retry plan."""
...
PLAN = BASE / "runtime" / "band-deck-switch-skill-profile-retry2-r1-plan.json"
...
def main() -> int:
    ...
    require(plan["scenario_id"] == "band-deck-switch-anonymous-skill-profile-retry2-r1" and plan["status"] == "ready-for-observation", "unexpected scenario")
    ...
    require([(action["kind"],action["marker"]) for action in plan["actions"]] == [
        ("tap","advance-result-detail-to-exp"),("tap","open-band-confirmation-without-live-start"),
    ...
    print("verified anonymous band deck Skill profile Retry-2 plan: result-detail actions=7 live-start=false resources=0")
    return 0
