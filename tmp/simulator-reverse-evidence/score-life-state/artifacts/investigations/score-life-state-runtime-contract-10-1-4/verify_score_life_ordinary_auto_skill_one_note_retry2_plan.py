#!/usr/bin/env python3
"""Fail closed unless Retry-2 is predeclared after the non-promoted Retry-1 trace."""
...
PLAN = BASE / "runtime" / "ordinary-auto-skill-one-note-retry2-r1-plan.json"
...
def main() -> int:
    ...
    require(plan["scenario_id"] == "ordinary-auto-skill-one-note-retry2-r1", "unexpected retry scenario")
    ...
    require(plan["actions"] == [
        {"kind":"tap","marker":"start-locked-ordinary-auto-live-retry2","x":1240,"y":590,"delay_ms":1000},
        {"kind":"wait_until_capture_complete","marker":"observe-complete-auto-live-retry2","timeout_ms":180000,"required_one_note_leave_count":979,"required_skill_finished_leave_count":6},
    ...
    require(any("nine remaining" in item for item in plan["preconditions"]), "Retry-1 resource provenance omitted")
    ...
    print("verified ordinary Auto Skill/one-note Retry-2 plan: remaining=9 resource=-1/-1 premium=0 supervised-transport-required")
    return 0
