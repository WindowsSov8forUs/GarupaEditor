#!/usr/bin/env python3
"""Observe anonymous Skill and one-note state in the locked ordinary Auto Live."""
...
for(const name of ['SituationSkillManager.AddSituationSkillToPlayList','SituationSkillManager.executeBeginSkillProcess','SituationSkillManager.executeFinishingSkillProcess','SituationSkillManager.processOfSkillTriggered','SituationSkillManager.playOnceEffectSkill','SituationSkillManager.processOfSkillFinished']){{hook(name,{{onEnter(args){{this.self=args[0];emit(name+'.enter',{{skill:skillState(this.self),data:skillData(args[1])}});}},onLeave(){{emit(name+'.leave',{{skill:skillState(this.self)}});}}}});}}
