...
- `BS01–BS36` partial oracle: 3 static-confirmed, 20 partial, 13 blocked; `unknown_fields=146`, `blocking_findings=89`.
...
The initialization-profile Retry trace is privacy-minimized and contains 11 contiguous events. For `poppin_shuffle_special.bms` / `special`, it observes music ID 3, score level 27, five deck-situation and character objects without their contents, Miss/Bad damage `-100/-50`, Life `1000/1000/2000`, max Note `979`, total parameter `0x483C8A31`, score-level rate `0x3F9C28F6`, zero event parameter, ordinary base score `0x4434718E` and zero bonus base. An independent Float32 recomputation matches both rate and base bits. Account identifiers, room fields, display strings and deck element contents are absent. This closes only the ordinary initialization values; deck member accumulation, HABAHIRO initialization and nonzero event/master profiles remain D23 blockers.

The BS01–BS36 oracle remains fail-closed without pretending closure: BS05, BS06 and BS11 are fully confirmed from direct 10.1.4 static evidence; 20 cases contain only directly supported static/R1/chart-count projections plus explicit unknown fields or blockers; 13 cases remain blocked with no expected projection. BS01 now consumes the ordinary initialization profile and has `unknown_fields=[]`, while its deck-member-row D23 blocker remains explicit. The fixed-event oracle records 146 unknown fields and 89 blocking findings overall. Fever transitions, multiple/overlapping Skill, guard/Never Die, remaining deck/start-data/master rows and final `closure.json` remain open. The five ABI-unsafe fields stay unconsumed. The business gate remains open and no trace or partial oracle authorizes production implementation.
