...
本目录冻结 `jp.co.craftegg.band` 10.1.4（code 230，`arm64-v8a`）资源与渲染阶段的静态、资源、ordinary运行时、实体frame manifest与显式HAB降级交付证据。Reverse 来源提交为 `e06940fb8f1e0519e49db3951cf5ed587e6edc2c`；逐文件字节数和 SHA-256 见 `manifest.json`。
...
`verify.mjs`校验734个冻结文件的Reverse commit/source working tree/copy/index四方字节、逐文件SHA-256、目录文件集、样本身份、673/32/19静态计数、资源/HUD/动画/ScoreUp计数、87,364-event ordinary R1、87,037-event geometry R2、10个setter target/510 mesh owner/80 line owner、1个current sync-line portable profile、7个physical frame manifest、12项current external HAB资源、H/D/PR closure、55/2/13 exact计划、2/12/179降级HAB决策及delivery/exact双轨门限。
