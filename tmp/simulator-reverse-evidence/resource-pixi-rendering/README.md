...
本目录冻结 `jp.co.craftegg.band` 10.1.4（code 230，`arm64-v8a`）资源与渲染阶段的静态、资源、ordinary运行时、实体frame manifest与显式HAB降级交付证据。Reverse 来源提交为 `ab5cc366a4a03d24a215e379849824e5ddf5f72f`；逐文件字节数和 SHA-256 见 `manifest.json`。
...
- final R6：五条clean全窗口trace共190,401 events/2,492 aggregate frames/26 observed owners；按当时证据只新增All Perfect ExecUpdate active gate，未观察路线未外推。
- final R7：130个byte-pinned owner target、七条confirmed trace共625,192 events/3,480 aggregate frames/51 observed owners/21 setters；包含Poppin' Shuffle Special双Note窗口、earliest field、Demo HUD与真实Auto Live AddScore/Score/Life/Skill。current static profile直接解析8个Shader program并锁定Advanced 42/120、threshold、完整HUD方法/scene/controller/clip；21个remaining PR与PR01–PR40 evidence authorization全部confirmed，unknown/blocker为0。
...
- 设备只读 cache、锁定 APK、实体PNG和外部资源字节只用于 Reverse 取证；本包仅提交最小结构化证据与匿名压缩R1–R7。
...
`verify.mjs`校验850个冻结文件的Reverse commit/source working tree/copy/index四方字节、逐文件SHA-256、目录文件集、样本身份、673/32/19静态计数、资源/HUD/动画/ScoreUp计数、87,364-event ordinary R1、87,037-event geometry R2、10个setter target/510 mesh owner/80 line owner、1个current sync-line portable profile、1个ordinary正交projection profile、1个ordinary Note geometry producer profile、1个Note child lifecycle profile及13个隔离ARM64 slice、30个R4 target/6个新增Slide slice/118,152-event与1,258-frame aggregate R4及其保守授权profile、35个R5 target/30,975-event/1,059-frame aggregate R5及5-route profile、190,401-event/2,492-frame/26-owner final R6及其保守profile、130个R7 target/625,192-event/3,480-frame/51-owner/21-setter final R7、current static/profile/40-case evidence closure、1个ordinary HUD runtime semantic profile、22个HUD setter target/ARM64 slice、19,888-event/631-frame visible R3及1个visible HUD/mask/animation profile、7个physical frame manifest、12项current external HAB资源、H/D/PR closure、55/2/13 exact计划、2/12/179降级HAB决策及delivery/exact双轨门限。
