#!/usr/bin/env python3
"""Build the version-10.1.4 static contract for resource and rendering owners."""
...
SOURCE_TARGETS = (
    "note-sprite-atlas-bindings",
    "note-sprite-combination-lookup",
    "flick-icon-rendering",
    "multiple-direction-side-rendering",
    "habahiro-lane-change",
    "hud-combo-rendering",
    "hud-combo-animation",
    "hud-judge-atlas-rendering",
    "hud-judge-color-rendering",
    "hud-life-rendering",
    "hud-score-result-rendering",
    "hud-score-up-overlay-rendering",
    "hud-skill-overlay-rendering",
    "hud-score-skill-overlay-rendering",
    "hud-judge-skill-overlay-rendering",
)

# Complete owner slices are intentional: these objects own visible state or its lifecycle.
CORE_OWNERS = (
    "NoteImageController",
    "NoteMesh",
    "NoteMeshAdvanced",
    ...
    "NoteMultipleDirectionalFlickBackLine",
    "NoteAddLongMultipleDirectionalFlickVisual",
    "NoteAddSlideMultipleDirectionalFlickVisual",
    "ButtonManager",
    "RhythmGameHabahiroFlashAnim",
    "Score",
    "Combo",
    "ComboNumber",
    "ComboNumber.<showCoroutine>d__9",
    "AllPerfectStatusAnimation",
    "AllPerfectStatusController",
    "InGameLifeGauge",
    "InGameSkillEffectDisplay",
    "JudgeTimingController",
    "AddScoreManager",
    "AddScoreObject",
    "AddScoreObject.<playCoroutine>d__11",
    "CE.Result",
    "CE.Result.<showCoroutine>d__31",
    "UISpriteNumber",
    "InGameUtility",
    "ColorUtility",
    "SkillEffectChangeableTextObject",
    "ScoreGaugeEffect",
)

EXTRA_METHODS = (
    ("NoteManager", "setupNoteSkin"),
    ("NoteBatchInformationListFactory", "combineNotes"),
    ("NoteBatchInformationListFactory", "get_IsMultiRangeNotes"),
    ("NoteBatchInformationListFactory", "IsEnableHabahiroChangeFlash"),
    ("AssetBundleNames", "get_HabahiroBundleName"),
    ("AssetBundleNames", "GetInGameSkinNoteSkinBundleName"),
    ("AssetBundleNames", "GetInGameSkinFieldSkinBundleName"),
    ("AssetBundleNames", "GetInGameSkinTapEffectSkinBundleName"),
)
...
ENUM_TYPES = (
    ...
    "NoteState",
    "NoteMesh.State",
    "UISpriteNumber.NUMBER_ALINE",
    "UISpriteNumber.TARGET_MODE",
    "UISpriteNumber.ARRANGEMENT_TYPE",
)
    ...
    for investigation in SOURCE_TARGETS:
        ...
        with path.open(encoding="utf-8", newline="") as source:
            for row in csv.DictReader(source, delimiter="\t"):
                ...
                if raw_address:
                    if raw_address.lower().startswith("0x"):
...
def main() -> int:
    ...
    result = {
        "schema_version": 1,
        "question": "What is the version-matched static contract for resource and rendering owners on 10.1.4 / 230?",
        "baseline": {
            "version_name": "10.1.3",
            "version_code": 229,
        ...
        "target": {
            ...
            "version_code": 230,
            "abi": "arm64-v8a",
            "base_apk_bytes": TARGET_BASE_APK.stat().st_size,
            "base_apk_sha256": digest(TARGET_BASE_APK),
            "arm64_split_apk_bytes": TARGET_ARM64_APK.stat().st_size,
            "arm64_split_apk_sha256": digest(TARGET_ARM64_APK),
            "libil2cpp_sha256": digest(TARGET_BINARY),
            "global_metadata_sha256": digest(TARGET_METADATA),
            "dump_cs_sha256": digest(TARGET_DUMP / "dump.cs"),
            "script_json_sha256": digest(TARGET_DUMP / "script.json"),
        },
        "method_layout_rebaseline": "closed" if counts(rows) == {"mapped": len(rows)} and counts(layouts) == {"unchanged": len(layouts)} and counts(enums) == {"unchanged": len(enums)} else "open",
    ...
    (HERE / "resource_pixi_rendering_static_contract.json").write_text(
