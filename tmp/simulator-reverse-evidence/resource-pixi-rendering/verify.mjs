...
const sourceCommit = "6191b424412df955ce0a7d9e85716ee94d1ed3dc";
...
check(manifest.entries.length === 737 && manifest.counts.totalEntries === 737, "Unexpected manifest entry count");
...
check(sums.size === 743, "Unexpected SHA256SUMS count");
...
console.log(`verified resource/Pixi delivery evidence: entries=737 methods=673 layouts=32 enums=19 resources=11026/57/100 profiles=8+4+4+5 plans=55/2/13 geometry=87037 line=1 projection=1 HAB=2/12/179 exact=open degraded=authorized H=28 D=18 PR=40 offline=closed delivery=closed exact-HAB=open production=true${validateIndex ? " index=checked" : ""}`);
