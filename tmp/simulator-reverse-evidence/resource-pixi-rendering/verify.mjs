...
const sourceCommit = "b8a749bb3fff2106237336e011a8e976a58ef94d";
...
check(manifest.entries.length === 700 && manifest.counts.totalEntries === 700, "Unexpected manifest entry count");
...
check(manifest.offlineWorkGate.status === "closed" && manifest.offlineWorkGate.unknownStaticWork.length === 0 && manifest.offlineWorkGate.unknownFields.length === 0, "Offline work gate differs");
...
check(sums.size === 699, "Unexpected SHA256SUMS count");
...
check(closure.status === "offline-work-gate-closed-server-required-gate-open" && closure.offline_work_gate === "closed" && closure.rendering_gate === "open" && closure.production_authorization === false, "Offline closure differs");
...
console.log(`verified resource/Pixi offline evidence: entries=700 methods=673 layouts=32 enums=19 resources=11026/57/100 profiles=8+4+4+5 H=28 D=18 PR=40 offline=closed rendering=open production=false blockers=S01,S02,S03${validateIndex ? " index=checked" : ""}`);
