...
- 功能实现完整，不显示运行时“Approximate”标签，不暴露approximation flag；
- HA-D01–HA-D12及parity差异只保留在任务书、验收记录和证据矩阵文本中。

本结论不宣称原作UnityFS、natural runtime owner/setter顺序、Root_effect原clip、GPU raster或original physical frame一致；这只影响文本中的证据/parity说明，不构成功能降级。
...
- Pixi只消费typed semantic commands，不用ticker创作领域时间；pause通过停止engine update冻结；production command和场景中不存在approximation说明对象。
- dispose后renderer object count为0；host只接受完整`current-external-complete`路线，旧degraded profile在创建production engine前失败关闭。
...
- 6,130 frames
- 217,595 commands
- digest：`74d11cf3742de6e955a46ddd0f5d1b5c8e620f74e3e52502a7feae364f3ad8b5`
- lifecycle order：`flash-start → field change → complete`，无运行时模式标签command
- 正向检查：179-row atlas key、long-flash binding、42/120 mesh、field mask、0 owner release

兼容回归：

- legacy degraded HABAHIRO：backend contract保留，但production host原子拒绝且0 owner；
...
npm.cmd run simulator:test:habahiro
...
HA-D01–HA-D12只作为文本差异边界，不进入production UI、snapshot、fidelity或semantic command。Exact HABAHIRO parity仍为`open-not-claimed`；只有获得current UnityFS、natural HAB runtime和original frame新证据后，才能另立exact parity任务，不得倒写本验收为原作逐帧复刻。
